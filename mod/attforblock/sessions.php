...
switch ($att->pageparams->action) {
    case att_sessions_page_params::ACTION_ADD:
        ...
        if ($formdata = $mform->get_data()) {
			...
			// if ($$sessions == NULL) {
				...
				// $error = "<script type='text/javascript'> alert('End date is less than start date'); return false; </script>";
				// echo $error;
			// }
			// else {
				$att->add_sessions($sessions);
				//redirect($url, get_string('sessionsgenerated','attforblock'));
				redirect($att->url_manage(), get_string('sessionsgenerated','attforblock')); // added by Qurrat-ul-ain
			//}
        }
        break;
    ...
    case att_sessions_page_params::ACTION_CHANGE_DURATION:
        ...
        if ($formdata = $mform->get_data()) {
			...
			$description = $formdata->sdescription;
            $att->update_sessions_duration($sessionsids, $duration, $description); // Added by Qurrat-ul-ain Babar (17th Jan, 2014)
            redirect($att->url_manage(), get_string('sessionupdated','attforblock'));
        }
...
function construct_sessions_data_for_add($formdata) {
    ...
    if (isset($formdata->addmultiply)) {
        ...
        //$wdaydesc = array(0=>'Mon', 'Tue', 'Wed', 'Thu', 'Fri');
        ...
        while ($sdate < $enddate) {
            if($sdate < $startweek + WEEKSECS) {
                ...
                if(array_key_exists($wdaydesc[$dinfo['wday']], $formdata->sdays)) {
					...
					// creates consecutive sessions - Added by Qurrat-ul-ain Babar (19th Dec, 2013)
					$sessionsperday = $formdata->sessionsperday;
					
					for($i=0; $i<$sessionsperday; $i++){
						 if ($i > 0)
						 {
							$sess->sessdate += $duration;
							$sess->sessdate += ($formdata->sdescription=="1")? 600 : 0;
		...
		/* if ($sessionsperday == '2')
		{
			...
			//$sessiondate += ($formdata->sdescription=="1")? 600 : 0;
			echo "Date: ".userdate($sessiondate, get_string('strftimedmyw', 'attforblock'));
			echo "Time: ".userdate($sessiondate, get_string('strftimehm', 'attforblock'))."<br/>";
		}
		else
		{
			echo "Date: ".userdate($formdata->sessiondate, get_string('strftimedmyw', 'attforblock'));
			echo "Time: ".userdate($formdata->sessiondate, get_string('strftimehm', 'attforblock'));
		}
		*/
