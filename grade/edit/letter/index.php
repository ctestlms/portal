...
///end
$sql = "select * from mdl_grade_items where courseid=$course->id AND itemtype = 'course' ";
$grade_ids = $DB->get_record_sql($sql);
// Commented off - Qurrat-ul-ain Babar (10th July, 2014)
// if ($grade_ids->locked == 1) {
    //echo "<div style='font-size:15px'><b>Sorry, The Gradebook is locked,the grade coundaries cannot be changed now.</b></div>";
	//notice("Sorry, The Gradebook is locked,the grade boundaries cannot be changed now.", $CFG->wwwroot . "/grade/report/grader/index.php?id=" . $course->id);
// }
//if viewing
if (!$edit) {
...
if (!$edit) {
    ...
    // Condition added - Qurrat-ul-ain Babar (10th July, 2014)
	if ($grade_ids->locked != 1) {
		echo $editlink;
	}
