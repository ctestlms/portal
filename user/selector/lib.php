...
class group_non_members_selector extends groups_user_selector_base {
	...
	//@Hina: group users. 1st level filtering.
		if($grp != ""){
		...
		//@Hina: end of LDAP filtering.
		
		// Added by Qurrat-ul-ain
		if($section != ""){
            
			$sql .= " AND u.id IN(SELECT userid FROM {role_assignments} WHERE roleid=5 and contextid IN (".$contextids.") )";
        }
