...
$PAGE->set_url('/blocks/custom_reports/editadvanced.php', array('course' => $course, 'id' => $id, 'schoolid' => $catg, 'status' => $status));
...
if ($usernew = $userform->get_data()) {
    ...
    if (!empty($usernew->idnumber)) {
        $eachRegNumber = explode("-", $usernew->idnumber);
        $eachRegNumber[sizeof($eachRegNumber) - 1] = str_pad($eachRegNumber[sizeof($eachRegNumber) - 1], 3, "0", STR_PAD_LEFT);
        $usernew->idnumber = implode("-", $eachRegNumber);
    }
    //end
    ...
    if ($user->id == $USER->id) {
        ...
        redirect("$CFG->wwwroot/blocks/custom_reports/user.php?schoolid=$catg&batch=$batch&status=$status");
    }
    //never reached
}
