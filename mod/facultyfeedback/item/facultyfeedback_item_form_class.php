...
abstract class facultyfeedback_item_form extends moodleform {
    public function definition() {
        ...
        if ($common['items']) {
            $mform->addElement('select',
                                'dependitem',
                                get_string('dependitem', 'facultyfeedback').'&nbsp;',
                                $common['items']
                                );
            $mform->addHelpButton('dependitem', 'depending', 'facultyfeedback');
            $mform->addElement('text',
                                'dependvalue',
                                get_string('dependvalue', 'facultyfeedback'),
        ...
        $position_select = $mform->addElement('select',
                                            'position',
                                            get_string('position', 'facultyfeedback').'&nbsp;',
                                            $positionlist);
        ...
        $mform->addElement('hidden', 'facultyfeedback', $common['facultyfeedback']);
        $mform->setType('facultyfeedback', PARAM_INT);
        ...
        if (!empty($item->id)) {
            $buttonarray[] = &$mform->createElement('submit',
                                                    'update_item',
                                                    get_string('update_item', 'facultyfeedback'));

            $buttonarray[] = &$mform->createElement('submit',
                                                    'clone_item',
                                                    get_string('save_as_new_item', 'facultyfeedback'));
        } else {
            $mform->addElement('hidden', 'clone_item', 0);
            $mform->setType('clone_item', PARAM_INT);
            $buttonarray[] = &$mform->createElement('submit',
                                                    'save_item',
                                                    get_string('save_item', 'facultyfeedback'));
        }
