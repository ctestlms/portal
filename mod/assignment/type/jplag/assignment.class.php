 ...
 *
 */
class assignment_jplag extends assignment_base {

	function assignment_jplag($cmid='staticonly', $assignment=NULL, $cm=NULL, $course=NULL) {
	...
	function view_upload_form() {
		...
		if ($this->can_upload_file($submission)) {
			...
			echo $OUTPUT->single_button(new moodle_url('/mod/assignment/type/jplag/upload.php', array('contextid'=>$this->context->id, 'userid'=>$USER->id)), $str, 'get');
		}
	}
	...
	function print_student_answer($userid, $return=false){
		...
		if ($this->notes_allowed() and !empty($submission->data1)) {
			$link = new moodle_url("/mod/assignment/type/jplag/notes.php", array('id'=>$this->cm->id, 'userid'=>$userid));
	...
	function print_user_files($userid=0, $return=false) {
		...
		if ($this->notes_allowed() and !empty($submission->data1) and !empty($mode)) { // only during grading
			...
			$npurl = $CFG->wwwroot."/mod/assignment/type/jplag/notes.php?id={$this->cm->id}&amp;userid=$userid&amp;offset=$offset&amp;mode=single";
	...
	function upload_notes() {
		...
		$mform = new mod_assignment_jplag_notes_form();
	...
	function upload_file($mform, $options) {
		...
		$this->view_header(get_string('upload'));
		add_to_log($this->course->id, 'assignment', 'upload error',
                    'view.php?a='.$this->assignment->id, $this->assignment->name, $this->cm->id);//Added By Hina Yousuf
		echo $OUTPUT->notification(get_string('uploaderror', 'assignment'));
		echo $OUTPUT->continue_button($returnurl);
		$this->view_footer();
		die;
...
class mod_assignment_jplag_notes_form extends moodleform {
...
class mod_assignment_jplag_response_form extends moodleform {
	function definition() {
		$mform = $this->_form;
