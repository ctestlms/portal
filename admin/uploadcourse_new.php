...
// -----------------------------------------------------------------------------------------------------
// Updated for Moodle 2 APIs by Mark Johnson <mark.johnson@tauntons.ac.uk>
// templates, topics and teachers are currently disabled as they have not been updated.
// Date: 19/4/10
...
// Some Regex constants
    ...
    $PAGE->set_url('/admin/uploadcourse_new.php');
    ...
    function validateas($value, $validatename, $lineno, $fieldname = '') {
        ...
        (isset($validate[$validatename])) or csverror('Coding Error: Unvalidated field type: "'.$validatename.'"','uploadcourse_new.php?sesskey='.$USER->sesskey);
        ...
        case 1: // String
                if (($maxlen = $format[1])!=0)  // Max length?
                    (strlen($value) <=$format[1]) or csverror('Invalid value for field '.$fieldname.' (length &gt; '.$format[1].'). '.
                                      ...
                                      'uploadcourse_new.php?sesskey='.$USER->sesskey);
        ...
        case 6: // User ID or Name (Search String)
            if(!empty($value)){
                $value=trim(clean_param($value,PARAM_TEXT));var_dump($value);
                if(count($user=get_record('user','idnumber',$value))==1){
                    $value=$user->id;
                }else{
                    csverror('Invalid value for field '.$fieldname.' (No User with IDnumber '.$value.'). '.
                              get_string('erroronline', 'error', $lineno) .". ".
                              get_string('processingstops', 'error'),
                              'uploadcourse_new.php?sesskey='.$USER->sesskey);
                }
        ...
        default:
            csverror('Coding Error: Bad field validation type: "'.$fieldname.'"','uploadcourse_new.php?sesskey='.$USER->sesskey);
        break;
        }
    ...
    function userexists_ex($userid) {
        ...
        return $DB->record_exists('user', array('deleted' => 0, 'id' => $userid));
    }
    ...
    function fastcreatecourse_ex($hcategory, $course, $header, $validate) {
        ...
        foreach ($header as $i => $col) {
            ...
            if($validate[$col][0] == 1 || ($validate[$col][0] == 4  && !checkisint($temparray[0]) ) ) { //String or Domain with strings
                $dataitem = clean_param($course[$col], PARAM_TEXT);
            } else {
                $dataitem = clean_param($course[$col], PARAM_INT);
            }            
            $coursedata->{$col} = $dataitem;
        }
...
/*    if (!confirm_sesskey()) {
    ...
    $struploadcourse_news = 'Upload Courses';
...
/// Print the header
    ...
    print_header("$site->shortname: $struploadcourse_news", $site->fullname,
                 "<a href=\"index.php\">$stradministration</a> -> $struploadcourse_news");
...
/// If a file has been uploaded, then process it
    ...
    if ($um->preprocess_files()) {
        ...
        if (!isset($um->files['coursefile']))
            csverror('Upload Error!', 'uploadcourse_new.php?sesskey='.$USER->sesskey);
        ...
        if ($fp = fopen($filename, "w")) {
            ...
            csverror('File I/O Error! (1)', 'uploadcourse_new.php?sesskey='.$USER->sesskey);
        }

        if (!$fp = fopen($filename, "r")) {
            csverror('File I/O Error! (2)', 'uploadcourse_new.php?sesskey='.$USER->sesskey);
        }
        ...
        $validate = array(  'fullname' => array(1,254,1), // Validation information - see validateas function
                            ...
                            'credithours'=>array(1,10,1));

        $header = fgetcsv($fp, 1024);
        // check for valid field names

        if (($header[0]==null)&&(count($line)<1)) // Blank Line?
            csverror('First line must be the CSV header', 'uploadcourse_new.php?sesskey='.$USER->sesskey);

        foreach ($header as $i => $h) {
            if ($h==null)
                csverror('Null CSV columns are not permitted in header', 'uploadcourse_new.php?sesskey='.$USER->sesskey);
                ...
                if (!(isset($required[$h]) || isset($optional[$h])))
                    csverror(get_string('invalidfieldname', 'error', $h), 'uploadcourse_new.php?sesskey='.$USER->sesskey);
        ...
        foreach ($required as $key => $value) {
            if ($value != true)
                csverror(get_string('fieldrequired', 'error', $key), 'uploadcourse_new.php?sesskey='.$USER->sesskey);
        }
        ...
        while (($line = fgetcsv($fp, 1024)) !== false) {
            if (($line[0]!=null)||(count($line)>1)) { // Blank Line?
                ...
                if (isset($coursetopics)) { 
                    $coursetocreate['topics'] = $coursetopics;
                } else {
                    $coursetocreate['topics'] = '';
                }
        ...
        if ((!isset($bulkcourses)) or (count($bulkcourses)==0))
            csverror('No courses were parsed from CSV', 'uploadcourse_new.php?sesskey='.$USER->sesskey);
        else
        ...
        notify('It is recommended that you run ANALYZE and OPTIMIZE (or equivalent for your DBMS) on the course, course_categories, course_sections asnd user_teachers tables.');
...
/// Print the form
    $OUTPUT->heading('Upload Courses');
    $maxuploadsize = get_max_upload_file_size();
    echo '<center>';
    echo '<form method="post" enctype="multipart/form-data" action="uploadcourse_new.php">'.
