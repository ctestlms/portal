...
class grade_item extends grade_object {
    ...
    public function is_overridable_item() {
       // return !$this->is_outcome_item() and ($this->is_external_item() or $this->is_calculated() or $this->is_course_item() or $this->is_category_item());
        return !$this->is_outcome_item() and ($this->is_external_item() or $this->is_calculated()); // Makes the course and category totals non-editable -- updated by Qurrat-ul-ain Babar (09th April,2014)
    }
     ...
     */
    public function bounded_grade($gradevalue, $oldvalue = null) {
        global $CFG;
        ...
        if ($this->gradetype == GRADE_TYPE_SCALE) {
            ...
            // 1.5 is rounded to 2 ;-)
            return (int) bounded_number($this->grademin, round($gradevalue + 0.00001), $this->grademax, $oldvalue);
        }
		...
		// save the 0.00 added in the quickediting mode - added by Qurrat-ul-ain (15th Jan, 2014)
        if (bounded_number($this->grademin, $gradevalue, $grademax, $oldvalue) != "") {
            return (float) bounded_number($this->grademin, $gradevalue, $grademax, $oldvalue);
        } else {
            echo "";
        }
     ...
     * @return bool success
     */
    public function update_final_grade($userid, $finalgrade = false, $source = NULL, $feedback = false, $feedbackformat = FORMAT_MOODLE, $usermodified = null, $oldvalue = null) {
        global $USER, $CFG,$DB;
        ...
        $grade->grade_item = & $this; // prevent db fetching of this grade_item
			if ($grade->grade_item->itemname == "ABS/I Grade"  ) {
				echo "<br/>ABS condition is fulfilled<br/>";
				
                $itemid = $DB->get_record_sql("select * from {grade_items} where courseid=$this->courseid and itemtype = 'course'");
                $cgrade = new grade_grade(array('itemid' => $itemid->id, 'userid' => $userid)); //$cgrade->grade_item = & $this; // prevent db fetching of this grade_item
                if (empty($usermodified)) {
                    $cgrade->usermodified = $USER->id;
                } else {
                    $cgrade->usermodified = $usermodified;
                }
                $cgrade->finalgrade = "";              
                $cgrade->update($source);
            } 
        if (empty($usermodified)) {
        ...
        if ($finalgrade !== false) {
            ...
            $grade->finalgrade = $this->bounded_grade($finalgrade, $oldvalue);
        }
        ...
        if (empty($grade->id)) {
            ...
            ///////////////Added By Hina Yousuf to change the coursetotal to zero  when student is absent
            if ($grade->grade_item->itemname == "ABS/I Grade"  ) {               
                $itemid = $DB->get_record_sql("select * from {grade_items} where courseid=$this->courseid and itemtype = 'course'");
                $cgrade = new grade_grade(array('itemid' => $itemid->id, 'userid' => $userid)); //$cgrade->grade_item = & $this; // prevent db fetching of this grade_item
                if (empty($usermodified)) {
    ...
    public function get_displaytype() {
        ...
        if ($this->display == GRADE_DISPLAY_TYPE_DEFAULT && $this->itemtype == "course") {
            return grade_get_setting($this->courseid, 'displaytype', $CFG->coursetotal_grade_displaytype);
        } elseif ($this->display == GRADE_DISPLAY_TYPE_DEFAULT && $this->itemtype != "course") {
