...
$url = new moodle_url('/mod/facultyfeedback/export.php', array('id'=>$id));
if ($action !== false) {
...
if ($action == 'exportfile') {
    if (!$exportdata = facultyfeedback_get_xml_data($facultyfeedback->id)) {
        print_error('nodata');
    }
    @facultyfeedback_send_xml_data($exportdata, 'facultyfeedback_'.$facultyfeedback->id.'.xml');
    exit;
}
...
function facultyfeedback_get_xml_data($facultyfeedbackid) {
    global $DB;

    $space = '     ';
    //get all items of the facultyfeedback
    if (!$items = $DB->get_records('facultyfeedback_item', array('facultyfeedback'=>$facultyfeedbackid), 'position')) {
        return false;
    }

    //writing the header of the xml file including the charset of the currrent used language
    $data = '<?xml version="1.0" encoding="UTF-8" ?>'."\n";
    $data .= '<FEEDBACK VERSION="200701" COMMENT="XML-Importfile for mod/facultyfeedback">'."\n";
    $data .= $space.'<ITEMS>'."\n";
...
function facultyfeedback_send_xml_data($data, $filename) {
