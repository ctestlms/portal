...
$url = new moodle_url('/mod/facultyfeedback/import.php', array('id'=>$id));
if ($choosefile !== false) {
...
$mform = new facultyfeedback_import_form();
...
if ($choosefile) {
    ...
    if (!$xmldata = facultyfeedback_load_xml_data($xmlcontent)) {
        print_error('cannotloadxml', 'facultyfeedback', 'edit.php?id='.$id);
    }

    $importerror = facultyfeedback_import_loaded_data($xmldata, $facultyfeedback->id);
    if ($importerror->stat == true) {
        $url = 'edit.php?id='.$id.'&do_show=templates';
        redirect($url, get_string('import_successfully', 'facultyfeedback'), 3);
        exit;
    }
}
...
///////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////
echo $OUTPUT->heading(get_string('import_questions', 'facultyfeedback'));
...
function facultyfeedback_load_xml_data($xmlcontent) {
    global $CFG;
    require_once($CFG->dirroot.'/lib/xmlize.php');

    if (!$xmlcontent = facultyfeedback_check_xml_utf8($xmlcontent)) {
        return false;
    }
...
function facultyfeedback_import_loaded_data(&$data, $facultyfeedbackid) {
    global $CFG, $DB;
...
function facultyfeedback_check_xml_utf8($text) {
    //find the encoding
