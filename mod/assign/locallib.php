...
class assign {
	...
	$update->jplagfile = $formdata->jplagfile; //Hina
    ...
    */
	...
	$mform->addElement('selectyesno', 'jplagfile', get_string('jplagfile', 'assign'));
 	$mform->addHelpButton('jplagfile', 'jplagfile', 'assign');
 	$mform->setDefault('jplagfile', 0);
        foreach ($this->submissionplugins as $plugin) {
	...
	//Added By Hina yousuf
        if ($this->instance->jplagfile == 1) {
            $o.=$this->type . '<form id="options" action="JPlag.php?id="' . $this->assignment->id . ' method="post">
		 <input type="button"  value="JPlag Submission" onClick="jplag()"		
		</form>';
            $assign_id = $this->instance->id;
            $submissionDir = '/moodledata1/' . $COURSE->id . '/' . $assign_id . '/' . $USER->id;
            ?><script>
                function jplag()
                {	
                    var dir = "<?php echo $submissionDir; ?>";
                    var aid = "<?php echo $this->instance->id; ?>";
                    window.open('JPlag.php?subDir='+dir+'&id='+aid);
                }
            </script><?php
        }
        //end
