...
class enrol_manual_potential_participant extends user_selector_base {
	...
	//@khyam: getting parameters for filtering
        ...
        $section = optional_param('section', "", PARAM_ALPHANUM); // Added by Qurrat-ul-ain
		$contextids = optional_param('contextids', "", PARAM_INT); // Added by Qurrat-ul-ain
                //@khyam: end.          
		...
		if($section != ""){
			// echo "Course Fullname: ".$course->fullname."<br/>";
			// echo "Context ID: ".$contextids."<br/>"; 
            
			$wherecondition .= " AND u.id IN(SELECT userid FROM {role_assignments} WHERE roleid=5 and contextid IN (".$contextids.") )";
        }
