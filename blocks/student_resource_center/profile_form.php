...
class profile_form extends moodleform {
    ...
    function definition() {
        ...
        $mform =& $this->_form;
      $freeze = false;
      if($DB->record_exists('regform', array('userid'=>$USER->id, 'freeze_edit' => 1))){
      $freeze = true;    
      }
        if($DB->record_exists('regform', array('userid'=>$USER->id))){
         $sql1= 'SELECT * FROM {regform} where userid ='.$USER->id;
      ...
      //  print_object($result);
        ...
        $mform->addElement('hidden','subgroup',$USER->user_subgroup);
...
$mform->addRule('thirdpassingyear', 'exceeds maximum length', 'maxlength', 4, 'client');       
 $mform->addRule('thirdpassingyear', get_string('error'), 'required', null, 'client');
      ...
      //  $mform->addRule('fourmajorsubjects', get_string('error'), 'required', null, 'client');
        ...
        if($freeze==true)
        {
            $mform->hardFreeze('thirddegreename');
            $mform->hardFreeze('thirdpassingyear');
            $mform->hardFreeze('thirdtotalcgpa');
            $mform->hardFreeze('thirdcgpaobtained');
            $mform->hardFreeze('thirdcgpapercentage');
            $mform->hardFreeze('thirdunivarr');
            $mform->hardFreeze('thirdmajorsubjects');
             
            $mform->hardFreeze('fourdegreename');
            $mform->hardFreeze('fourpassingyear');
            $mform->hardFreeze('fourtotalcgpa');
            $mform->hardFreeze('fourcgpaobtained');
            $mform->hardFreeze('fourcgpapercentage');
            $mform->hardFreeze('fouruniversity');
            $mform->hardFreeze('fourmajorsubjects');
        }
   } 
      ...
      $guardian[] = $mform->createElement('static', 'reldes', 'for Example','<font color="black">For Example: Brother</font>' );
        ...
        if($freeze==true)
        {
            $mform->hardFreeze('rollno');
            $mform->hardFreeze('meritno');
            $mform->hardFreeze('dob');
            $mform->hardFreeze('gender');
            $mform->hardFreeze('religion');
            $mform->hardFreeze('nicarr');
            $mform->hardFreeze('bloodgroup');
            $mform->hardFreeze('fatherprofarr');
            $mform->hardFreeze('fatherrank');
            $mform->hardFreeze('fatherservice');
            $mform->hardFreeze('domiarr');
            $mform->hardFreeze('province');
            $mform->hardFreeze('dispatchmode');
            $mform->hardFreeze('admissiontype');
            $mform->hardFreeze('foreignstudent');
            $mform->hardFreeze('currphoneno');
            $mform->hardFreeze('permanentaddress');
            $mform->hardFreeze('permanentcity');
            $mform->hardFreeze('perphoneno');
            $mform->hardFreeze('fathermobileno');
            $mform->hardFreeze('emergencyno');
            $mform->hardFreeze('faxno');
            $mform->hardFreeze('firstdegreename');
            $mform->hardFreeze('firstpassingyear');
            $mform->hardFreeze('firsttotalmarks');
            $mform->hardFreeze('firstmarksobtained');
            $mform->hardFreeze('firstpercentage');
            $mform->hardFreeze('boardarr');
            $mform->hardFreeze('firstmajorsubjects');
            $mform->hardFreeze('seconddegreename');
            $mform->hardFreeze('secondpassingyear');
            $mform->hardFreeze('secondtotalmarks');
            $mform->hardFreeze('secondmarksobtained');
            $mform->hardFreeze('secondpercentage');
            $mform->hardFreeze('secondboard');
            $mform->hardFreeze('secondmajorsubjects');
            $mform->hardFreeze('guardianarr');
           // $mform->hardFreeze('guardianrelation');
        ...
        $buttonarray=array();
        if($DB->record_exists('regform',array('userid'=>$USER->id,'freeze_edit'=>0))){
            
        $buttonarray[] =$mform->createElement('submit', 'submitbutton', "Submit",array('onClick'=>"return confirm('Are you sure you want to submit your data to the NUST Exam Branch?');"));
            //,array('onClick'=>"jsfunction()")
        
        }
        else if($DB->record_exists('regform',array('userid'=>$USER->id,'freeze_edit'=>1))){
           $buttonarray[] = &$mform->createElement('submit', 'submitbutton', "Print Form"); 
        }
        if($DB->record_exists('regform',array('userid'=>$USER->id,'freeze_edit'=>0)) || !$DB->record_exists('regform',array('userid'=>$USER->id))) {
        $buttonarray[] = &$mform->createElement('submit', 'submitbutton', "Save Profile");
        }
        
        
        $mform->addGroup($buttonarray, 'buttonar', '', array(' '), false);
        $mform->addElement('static', 'imp', '<b>Important:</b>','<font color="red"> Once you click on <b>"Submit"</b> button, you will not be able to make any changes. <br/> If you need to make any change to your profile, please contact exam branch of your respective School to make changes for you.</font>');
