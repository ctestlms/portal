...
class feedback_item_multichoice extends feedback_item_base {
    ...
    public function excelprint_item(&$worksheet, $row_offset,
        ...
        global $CFG,$DB,$SESSION;//Adde By Hina Yousuf
        ...
        //frage schreiben
        $worksheet->write_number($row_offset, 0, $SESSION->count, $xls_formats->head2);//Added By Hina Yousuf
        $worksheet->write_string($row_offset, 0, $item->label, $xls_formats->head2);
        $worksheet->write_string($row_offset, 1, $analysed_item[1], $xls_formats->head2);
        $SESSION->count=$SESSION->count+1;//Added By Hina Yousuf
        if (is_array($data)) {
