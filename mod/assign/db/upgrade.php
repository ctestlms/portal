...
function xmldb_assign_upgrade($oldversion) {
    ...
    if ($oldversion < 2012112901) {
        ...
        // Define field jplagfile to be added to assign
        $table = new xmldb_table('assign');
        $field = new xmldb_field('jplagfile', XMLDB_TYPE_CHAR, '255', null, null, null, null, 'revealidentities');

        // Conditionally launch add field jplagfile
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }

        // assign savepoint reached
        upgrade_mod_savepoint(true, 2012112901, 'assign');
    }
