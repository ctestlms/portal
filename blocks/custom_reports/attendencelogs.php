...
require_once('./view_attendance_report_form1.php');
...
function get_all_sessions($course,$startdate,$enddate)
{
		...
		$qry ="select id from {attforblock} where course=$course";
		$att = $DB->get_record_sql($qry);
...
function get_all_session_logs($course,$sessionId)
{
	 ...
	 //AND time >=$startDate and time<=$endDate
	      $sql="SELECT *, time as timetaken  FROM {log}  WHERE   course = ".$course."  AND action  = 'attendance taken' 
		AND    `url` LIKE '%".$sessionId."%'  
		ORDER BY `id` ASC";
        $sessionsTime=$DB->get_records_sql($sql,array());
        return $sessionsTime;
...
if(($courses && $categoryid==2) or $export or $sort){
	$mform = new mod_custom_reports_view_attendance_report_form('attendencelogs.php', array('courses'=>$courses, 'categoryid'=>$categoryid,'report'=>$report));
	...
	if($fromform = $mform->get_data() or $export or $sort){
		...
		$sessionlogs  = get_all_sessions($courseId,$startdate, $enddate);
  ...
  <tr>
    ...
    <td>Action</td>
    <td>URL</td>
    <td>Attendence Taken Date/Time</td>
     
     
  </tr>
  <?php $i=0;
   foreach($sessionlogs as $logData) {
	  
	   $courseDetails = $DB->get_record_sql("select category,fullname from {course} where id = ".$courseId."  ");
	   $logsData=  get_all_session_logs($courseId,$logData->id);
	   $sessionStartime = $logData->sessdate;
	   $sessionEndtime = $logData->sessdate+$logData->duration;
	   foreach($logsData as $logs ) { $i++;
  ...
  <tr>
    <td><?php echo $i;?></td>
    <td><?php echo $courseDetails->fullname; ?></td>
    <td><?php echo  date("d-m-Y",$logData->sessdate); ?></td>
    <td><?php echo "(".date("D",$logData->sessdate).")"." ".date("h:i",$logData->sessdate)."-".date("h:i",$logData->sessdate+$logData->duration)?></td>
    <td><?php echo $logs->action;?></td>
    <td><?php echo $logs->url;?></td>
    <td><?php echo  date("d-m-Y h:i:s",$logs->timetaken); ?></td>
 ...
 ?>
		...
		$table->head[] = 'Attendence Log Time';
		$table->align[] = 'center';
		$table->size[] = '';
		$table->headspan[] = 1;
	...
	/*	$table->head[] = 'Remarks';
		...
		foreach($logs as $logData) {
			print_r($logData);
			    ...
			    $courseDetails = $DB->get_record_sql("select category,fullname from {course} where id = '10589'  ");
				echo $category = $courseDetails->category;
				...
				$context = get_context_instance(CONTEXT_COURSE, '10589');
				...
				$coursefullname = explode("(", $courseDetails->fullname);
				$data[$i]["sno"] = $i+1;
				 
				$data[$i]["class"] = rtrim($coursefullname[1],")");
				$data[$i]["coursefullname"] = (!$export)?'<a href="'.$CFG->wwwroot.'/course/view.php?id=10589" target="_blank">'.$coursefullname[0].'</a>':$coursefullname[0];
				$data[$i]["dept"] =$department;
				$data[$i]["teacher"] = rtrim($course_teacher, "<br>"); 
				
				$data[$i]["logTime"] = date("m/d/Y h:i:s",$logData->timetaken);
...
/*		$datas[$i]["reqlec"] = "<b>Lecture</b>";
	...
	echo '<form method="post" action="attendencelogs.php?id=2" style="display: inline; margin: 0; padding: 0;">';
