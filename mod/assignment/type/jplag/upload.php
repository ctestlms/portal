...
session_start();//added By Hina Yousuf
...
$url = new moodle_url('/mod/assignment/type/jplag/upload.php', array('contextid'=>$contextid,
...
//end
$instance = new assignment_jplag($cm->id, $assignment, $cm, $course);
$submission = $instance->get_submission($formdata->userid, true);

$filemanager_options = array('subdirs'=>1, 'maxbytes'=>$assignment->maxbytes, 'maxfiles'=>$assignment->var1, 'accepted_types'=>  array('*.java', '*.cs','*.txt','*.c','*.cpp') 
, 'return_types'=>FILE_INTERNAL);
//$_SESSION['assignment']=$assignment->id;//Added By Hina Yousuf

$mform = new mod_assignment_jplag_form(null, array('contextid'=>$contextid, 'userid'=>$formdata->userid, 'options'=>$filemanager_options));
...
echo "<div align='center'><b>Files with extensions (.java, .scm,  .cpp,  .c,  .h, .hpp,  .txt,  .asc, .cs) are allowed only.</b></div><br/>";
