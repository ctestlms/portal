<?php
    ...
    $columns = array_merge(array('firstname', 'lastname', 'idnumber', 'user_group', 'user_subgroup','dn_path'), extracolumns,
   ...
   // print_object($usercount);
	...
	$table->head[] = $idnumber;
        $table->align[] = 'left';
        $table->head[] = $user_group;
        $table->align[] = 'left';
        $table->head[] = $user_subgroup;
        $table->align[] = 'left';
	$table->head[] = $dn_path;
        $table->align[] = 'left';
        $table->head[] = $city;
    ...
    if (!empty($table)) {
       ...
       // echo html_writer::end_tag('div');//close the no-overflow div
        echo $OUTPUT->paging_bar($usercount, $page, $perpage, $baseurl);
        if (has_capability('moodle/user:create', $sitecontext)) {
            echo $OUTPUT->heading('<a href="'.$securewwwroot.'/user/editadvanced.php?id=-1">'.get_string('addnewuser').'</a>');
        }
