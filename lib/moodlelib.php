...
function authenticate_user_login($username, $password) {
    ...
    foreach ($auths as $auth) {
	 ...
	 //bug removed last name repitition
        $string = $user->firstname;
        $find = $user->lastname;
        if(strstr($string, $find) ==true){
                //  $record = new stdClass();             
                  $user->firstname= str_replace($find, "", $string);
                  //$record->id=$user->id;
                  $DB->update_record("user", $user);
         }
//Added By Hina Yousuf  //reg no padding zeros
        if(!empty($user->idnumber)){
                        $eachRegNumber = explode("-",$user->idnumber);
                        $eachRegNumber[sizeof($eachRegNumber)-1] = str_pad($eachRegNumber[sizeof($eachRegNumber)-1],3,"0", STR_PAD_LEFT);
                        $user->idnumber = implode("-",$eachRegNumber);
