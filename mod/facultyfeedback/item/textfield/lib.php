...
class facultyfeedback_item_textfield extends facultyfeedback_item_base {
    ...
    public function build_editform($item, $facultyfeedback, $cm) {
        ...
        $this->item_form = new facultyfeedback_textfield_form('edit_item.php', $customdata);
    }
    ...
    public function get_analysed($item, $groupid = false, $courseid = false) {
        ...
        $values = facultyfeedback_get_group_values($item, $groupid, $courseid);
        if ($values) {
            $data = array();
    ...
    public function excelprint_item(&$worksheet, $row_offset,
                             $xls_formats, $item,
                             $groupid, $courseid = false,$noofusers = false) {
        global $SESSION;
        $analysed_item = $this->get_analysed($item, $groupid, $courseid);
        $worksheet->write_number($row_offset, 0, $SESSION->count, $xls_formats->head2);
        ...
        if (is_array($data)) {
            $worksheet->write_string($row_offset,
                                7,
                                get_string('entered', 'facultyfeedback'),
                                $xls_formats->value_bold);
            $worksheet->write_string($row_offset,
                                8,
                                get_string('notentered', 'facultyfeedback'),
                                $xls_formats->value_bold);
            $row_offset++;
...
//            }
        ...
        $worksheet->write_number($row_offset, 7, $sizeofdata, $xls_formats->default);
        $worksheet->write_number($row_offset, 8, ($noofusers-$sizeofdata), $xls_formats->default);
	...
	//Added By Hina Yousuf
        ...
        $analysed_item = $this->get_analysed($item, $groupid, $courseid);
                ...
                $content = $content.'<tr><td></td></tr><td width="30%"><b>'.$item->name.'</b></td>';
        $data = $analysed_item->data;
        if(is_array($data)) {
            $content = $content.'<td width="70%"><b>'.$data[0].'</b></td></tr>';
            $rowOffset++;
            $sizeofdata = sizeof($data);
            for($i = 1; $i < $sizeofdata; $i++) {
                $content = $content.'<tr><td width="30%"></td><td width="70%"><b>'. $data[$i].'</b></td></tr>';
                $rowOffset++;
            }
                $content.="</table>";
        }
    ...
    public function print_item_preview($item) {
        ...
        echo '<span class="facultyfeedback_item_textfield">';
