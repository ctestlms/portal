...
switch ($action) {
	...
	case 'getFace':
        ...
        //Extract faces from the uploaded file and return base64 encoded cropped images
        //Added by Junaid Malik
        $url = "/var/moodledata/1.jpg";
		$extension = end(explode(".", $_FILES['repo_upload_file']['name']));
        $url2 = "/var/moodledata/2.".$extension;
        move_uploaded_file($_FILES['repo_upload_file']['tmp_name'],$url2);
		$contents = file_get_contents($url2);
		unlink($url2);
		$im = imagecreatefromstring($contents);
		imagejpeg($im,$url);
        $count = face_count($url,'haarcascade_frontalface_alt.xml');
	...
	if ($count > 0) {
			...
			if ($width > 800) {
				$newwidth = $width/($width/800);
				$resize = true;
			}
			
			if ($height > 800) {
				$newheight = $height/($height/800);
				$resize = true;
			}
			
			if ($resize) {
				imagecopyresampled($im,$im,0,0,0,0,$newwidth,$newheight,$width,$height);
				imagejpeg($im,$url);
			}
			
            $ord = face_detect($url,'haarcascade_frontalface_alt.xml');
	    //$ord_eye_left = face_detect($url,'haarcascade_mcs_lefteye.xml');
	    //$ord_eye_right = face_detect($url,'haarcascade_mcs_righteye.xml');
            $cropped_faces = array();
		...
		$width_factor = 0.1*$face['w'];
        $height_factor = 0.25*$face['h'];
        
		
		$cropped_face = imagecreatetruecolor(200,200);
		//                 $dst_image , $src_image,int $dst_x,int $dst_y,int $src_x, int $src_y , int $dst_w , int $dst_h , int $src_w , int $src_h )
        imagecopyresampled($cropped_face,$im,0,0,$face['x']-$width_factor,$face['y']-$height_factor,200,200,$face['w']+($width_factor*2),$face['h']+($height_factor*2));
                  ob_start ();
                  imagejpeg ($cropped_face);
                  $val = ob_get_contents ();
                  ob_end_clean ();
		
		$val = "data:image/jpeg;base64," . base64_encode($val);
	...
	if ($i>0) {
		$result = array('eventFace'=>'facedetected','face_count'=>$i,'faces'=>$return);	
	}


        else {
            $result = array('eventFace'=>'noface');
        }
    ...
    case 'uploadface':
        ...
        // Save the selected face in moodledata folder
        // Added by Junaid Malik

        $param1 = required_param('face',PARAM_RAW);
        $b64 = $param1;
        $fn = md5(uniqid());
        $saveas = '/var/moodledata/' . $fn . '.jpg';
        $image = explode('base64,',$b64);
        file_put_contents($saveas, base64_decode($image[1]));
        $result = $repo->upload($fn,$maxbytes);
        echo json_encode($result);
        break;

	case 'uploadExistingFace':
		$result = $repo->get_user_picture();
		$im1 = imagecreatefromstring ($result);
		$url = "/var/moodledata20/1.jpg";
		imagejpeg($im1,$url);
        //file_put_contents($result,$url);
        $count = face_count($url,'haarcascade_frontalface_alt.xml');
	...
	if ($count > 0) {
		...
		//echo "Face width is " . $face['w'] . "and height is " . $face['h'];

		
		$width_factor = 0*$face['w'];
        $height_factor = 0*$face['h'];
        
		
		$cropped_face = imagecreatetruecolor(200,200);
		//if ($face['w']>70 && $face['h']>70) {
		//	imagecopyresampled($cropped_face,$im,0,0,0,0,200,200,100,100);			
		//}
		//imagecopyresampled($im,$im,0,0,0,0,300,300,$width,$height);
		//                 $dst_image , $src_image,int $dst_x,int $dst_y,int $src_x, int $src_y , int $dst_w , int $dst_h , int $src_w , int $src_h )
		//else {
			imagecopyresampled($cropped_face,$im,0,0,$face['x']-$width_factor,$face['y']-$height_factor,200,200,$face['w']+($width_factor),$face['h']+($height_factor));			
		//}
