...
class assign_grading_table extends table_sql implements renderable {
    ...
    public function __construct(assign $assignment, $perpage, $filter, $rowoffset, $quickgrading, $downloadfilename = null) {
	   ...
	   /////Added By Hina Yousuf//Jplag files
        if ($this->assignment->get_instance()->jplagfile) {
            $assign_id = $this->assignment->get_instance()->id;
            $course = $this->assignment->get_course()->id;
            mkdir("/moodledata1/$course");
            mkdir("/moodledata1/$course/$assign_id");
            mkdir("/moodledata1/$course/$assign_id/$USER->id");
           
            foreach ($userparams as $userid) {

                $submission = $DB->get_record('assign_submission', array('assignment' => $assign_id, 'userid' => $userid));
                //  echo $submission->id."-";
                $fs = get_file_storage();
                $files = $fs->get_area_files($this->assignment->get_context()->id, 'assignsubmission_file', 'submission_files', $submission->id, "id", false);

                $userrecord = $DB->get_record_sql("Select * from {user} where id=$userid ");
                foreach ($files as $file) {
                   
                    $filename = $userrecord->firstname . "_" . $userrecord->lastname . "_" . $file->get_filename();
                    
                    $contents = $file->copy_content_to("/moodledata1/$course/$assign_id/$USER->id/$filename");
                }
            }
        }
     //end JPlag Assignment
        // Select.
