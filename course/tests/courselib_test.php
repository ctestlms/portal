...
class courselib_testcase extends advanced_testcase {
    ...
    public function check_module_visibility($mod, $visibility, $visibleold) {
     ...
     public function test_auto_create_attenance_activity() {
        global $DB;
        $this->resetAfterTest(true);

        $course = $this->getDataGenerator()->create_course(array('shortname' => 'attendance activity', 'fullname' => 'Attendance activity test'));      
       
        $attendance = $DB->get_record('attforblock', array('course' => $course->id));
        $this->assertEquals($course->id, $attendance->course);
