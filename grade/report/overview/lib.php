...
class grade_report_overview extends grade_report {
    ...
    public function fill_table() {
        ...
        if ($courses = enrol_get_users_courses($this->user->id, false, 'id, shortname, showgrades')) {
		 ...
		 $this->showlettergrade = grade_get_setting($course->id, 'report_user_showlettergrade', !empty($CFG->grade_report_user_showlettergrade));

                if (!$this->showlettergrade) {
                    $data = array($courselink, grade_format_gradevalue($finalgrade, $course_item, true, GRADE_DISPLAY_TYPE_REAL));
                } else {
