...
//            echo get_string('average', 'feedback').': '.$avg.'</b>';
    ...
    public function excelprint_item(&$worksheet, $row_offset, $xls_formats, $item, $groupid, $courseid = false, $noofusers = false) {
        ...
        if (is_array($data)) {
            ...
            for ($i = 0; $i < $sizeofdata; $i++) {
                ...
                $worksheet->write_string($row_offset, $i + 2, trim($analysed_data->answertext) . ' (' . $analysed_data->value . ')', $xls_formats->value_bold);
...
//        }
    ...
    function pdfprint_item(&$content, $item, $groupid, $courseid = false) {
        ...
        if ($_SESSION['count'] == 1) {
            ...
            $content = $content . '<td width="10%"><b>' . get_string('score', 'feedback') . '</b></td>';
            $content = $content . '<td width="10%"><b>' . get_string('average', 'feedback') . '</b></td></tr>';
        }
        $rowOffset+=2;
        ...
        if (is_array($data)) {
            ...
            $content = $content . '<td width="10%"><b>' . $score . '</b></td>';
            $content = $content . '<td width="10%"><b>' . $avgs . '</b></td></tr>';
    ...
    function graph_item($item, $groupid, $courseid = false) {
        ...
        if ($_SESSION['count'] == 1) {
            ...
            for ($i = 0; $i < $sizeofdata; $i++) {
                ...
                if (strstr($data[0]->answertext, "Strongly Agree") == true || strstr($data[0]->answertext, "Excellent") == true) {
