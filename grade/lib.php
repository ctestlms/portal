...
abstract class grade_helper {
    ...
    public static function get_info_letters($courseid) {
        // DB var added -  Qurrat-ul-ain Babar (10th July, 2014)
		global $SITE, $DB;
		// Added by Qurrat-ul-ain Babar (10th July, 2014)
		$sql = "select * from mdl_grade_items where courseid=$courseid AND itemtype = 'course' ";
        ...
        if ($canmanage || $canmanageletters) {
			...
			$letterarray['view'] = new grade_plugin_info('view', new moodle_url('/grade/edit/letter/index.php', array('id' => $context->id)), get_string('view'));
			// Condition added for checking if grade book locked -  Qurrat-ul-ain Babar (10th July, 2014)
			if ($grade_ids->locked == 1) {
				// do not show edit link
			} else 
				$letterarray['edit'] = new grade_plugin_info('edit', new moodle_url('/grade/edit/letter/index.php', $param), get_string('edit'));
		...
		//'graph' => new grade_plugin_info('graph', new moodle_url('/grade/edit/graph/index.php', array('cid1'=>$courseid)), "Graph")///Added By Hina Yousuf on 19-6-2012
          //  );
        }  self::$letterinfo = $letterarray;
        if ($canviewgraph = has_capability('gradereport/grader:viewlettergrades', $context)) {
            $letterarray['graph'] = new grade_plugin_info('graph', new moodle_url('/grade/edit/graph/index.php', array('cid1' => $courseid)), "Graph"); ///Added By Hina Yousuf on 19-6-2012            
            self::$letterinfo = $letterarray;      
        } else {
