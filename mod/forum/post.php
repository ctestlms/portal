...
if ($fromform = $mform_post->get_data()) {
    ...
    if ($fromform->edit) {           // Updating a post
        ...
        if ($fromform->id = forum_add_new_post($addpost, $mform_post, $message)) {
			...
			//Added by Qurrat-ul-ain for Android SMS App (18th July, 2013)
			if (!empty($fromform->smsnow)) {
				$message .= get_string("postsmsnow", "forum");
				$timemessage = 4;
				
				$subscribedusers = forum_subscribed_users($course, $forum, 0, $modcontext, "u.*"); // gets the users subscribed to this forum
				$forumtitle = $discussion->name; // gets the forum subject line to be sent in sms
				
				
				$con = mysql_connect('10.3.72.51', 'root', 'MF1ICs50!'); // makes a connection to the DB
				global $num_user,$num_msgs;
				...
				//mysql_query("insert into mdl_sms_msg (Date,msg) values (Now(),'$forumtitle')");  // inserts the sms content into DB
			
				
				//$DB->set_debug(true);
				$sms = new stdClass();
                $sms->Date = date("Y-m-d");
				$sms->msg   = $forumtitle." \"Please do not reply. Check LMS portal for details.\"";
				//$sms->msg   = $forumtitle;
				
				//echo $sms->msgDate." \n";
				//echo $sms->msg." \n";
			
				$sms->Msg_Id = $DB->insert_record("sms_msg", $sms); //updated
				$smsID = $sms->Msg_Id;
				
				
				// Inserts the users phone numbers along with the corresponding sms ID in DB
				foreach($subscribedusers as $result)
				{
					...
					if(!empty($phone))
					{
						if (strpos($phone,'-') !== false) {
							//Removes string token "-" from phone number
							$phone_no = explode("-", $phone);
							$phone = $phone_no[1];
						}
						...
						/*$updatesenderid = new stdClass();
						$updatesenderid->id      = $contact->id;
						$updatesenderid->sender   = $USER->id;
						
						$DB->update_record("sms_contacts", $updatesenderid);
						
						
						$msgid = mysql_query("Select Msg_Id from mdl_sms_msg Order By Msg_Id DESC Limit 1");

						while($row = mysql_fetch_assoc($msgid))
						{
							$mid =  $row['Msg_Id'];
						}*/
						mysql_query("insert into mdl_sms_contacts (Msg_Id,sender_id,contacts,Date,Status,Retrys) values ('$smsID','$USER->id','$phone',NOW(), 'pending', '0')");		
						
					}
				}
				
				mysql_close($con);
				add_to_log($course->id, "forum", "send sms",
                      "discuss.php?d=$discussion->id", "$forumtitle", $cm->id);	
			} else {
				$message .= '<p>'.get_string("postaddedsuccess", "forum") . '</p>';
				$message .= '<p>'.get_string("postaddedtimeleft", "forum", format_time($CFG->maxeditingtime)) . '</p>';
		...
		$fromform->smsnow  = empty($fromform->smsnow) ? 0 : 1; 		//Added by Qurrat-ul-ain for Android SMS App (18th July, 2013)
