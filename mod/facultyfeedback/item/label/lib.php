...
class facultyfeedback_item_label extends facultyfeedback_item_base {
    ...
    public function build_editform($item, $facultyfeedback, $cm) {
        ...
        $this->item_form = new facultyfeedback_label_form('edit_item.php', $customdata);
    }
    ...
    public function print_item($item) {
        ...
        if (!$item->facultyfeedback AND $item->template) {
            $template = $DB->get_record('facultyfeedback_template', array('id'=>$item->template));
            ...
            $cm = get_coursemodule_from_instance('facultyfeedback', $item->facultyfeedback);
    ...
    //when provided $data submitted from facultyfeedback_show_edit
