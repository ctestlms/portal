...
$url = new moodle_url('/mod/facultyfeedback/analysis_to_pdf.php', array('id'=>$id));
if ($coursefilter !== '0') {
...
$sem_duration=$DB->get_records_sql("SELECT startdate from {course} c WHERE id =$course->id");
foreach($sem_duration as $sem_duratn){
$sem_durtn=$sem_duratn->startdate;
$weeks=18;//$sem_duratn->numsections;
}
...
if(!$items = $DB->get_records('facultyfeedback_item', array('facultyfeedback'=>$facultyfeedback->id, 'hasvalue'=>1), 'position')) {
    print_error('no_items_available_yet', 'facultyfeedback', $CFG->wwwroot.'/mod/facultyfeedback/view.php?id='.$id);
    exit;
}
...
// 	$content = $content.'<h1 align="center">Faculty Feedback</h1>';
	$content = $content.'<table cellpadding="2" border="0"><tr><td ><img src="pix/NUST_Logo.jpg" height="52" width="52" /> <font size="15"><b>&nbsp; &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Faculty Feedback</b><br/></font></td></tr></table>';
...
//end of pdf
function facultyfeedback_excelprint_detailed_head(&$worksheet, $xlsFormats, $items, $rowOffset) {
...
function facultyfeedback_excelprint_detailed_items(&$worksheet, $xlsFormats, $completed, $items, $rowOffset) {
    global $DB, $fstring;
    ...
    $colOffset++;
    foreach($items as $item) {
        $value = $DB->get_record('facultyfeedback_value', array('item'=>$item->id, 'completed'=>$completed->id));
        ...
        if($courseid == 0) $courseid = $facultyfeedback->course;
    }
