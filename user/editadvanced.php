...
//$school = $DB->get_record('course_categories', array('id'=>$schoolid), '*', MUST_EXIST);
...
//{
  //  echo $schoolid;
   //$catcontext = context_coursecat::instance($schoolid);
  //  $catcontext = get_context_instance(CONTEXT_COURSECAT, $schoolid);
//}
...
if ($id == -1) {
    ...
    if(!has_capability('moodle/user:update', $systemcontext)){
        ...
        require_capability('block/student_resource_center:addinstance', $catcontext);
...
//print_object($usernew);
    ...
    if ($user->id == $USER->id) {
       ...
       //     redirect("$CFG->wwwroot/blocks/student_resource_center/profile_edit.php?id=$user->id");
        }
    } else {
   
        session_gc(); // remove stale sessions
       
       if($DB->record_exists('role_assignments',array('roleid'=>21,'userid'=>$USER->id))){
           
       redirect("$CFG->wwwroot/user/profile_edit.php?userid=$user->id&schoolid=$schoolid");
        }
        else{
