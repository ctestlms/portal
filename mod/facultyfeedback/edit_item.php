...
$editurl = new moodle_url('/mod/facultyfeedback/edit.php', array('id'=>$cmid));

if (!$typ) {
    redirect($editurl->out(false));
}

$url = new moodle_url('/mod/facultyfeedback/edit_item.php', array('cmid'=>$cmid));
if ($typ !== false) {
    $url->param('typ', $typ);
}
if ($id !== false) {
...
if (! $cm = get_coursemodule_from_id('facultyfeedback', $cmid)) {
...
if ($typ == 'pagebreak') {
    facultyfeedback_create_pagebreak($facultyfeedback->id);
    redirect($editurl->out(false));
    exit;
}
...
if ($id and $item = $DB->get_record('facultyfeedback_item', array('id'=>$id))) {
    $typ = $item->typ;
} else {
    $item = new stdClass();
    $item->id = null;
    $item->position = -1;
    if (!$typ) {
        print_error('typemissing', 'facultyfeedback', $editurl->out(false));
    }
    $item->typ = $typ;
    $item->options = '';
}
...
$itemobj = facultyfeedback_get_item_class($typ);

$itemobj->build_editform($item, $facultyfeedback, $cm);
...
if ($itemobj->get_data()) {
    if ($item = $itemobj->save_item()) {
        facultyfeedback_move_item($item, $item->position);
        redirect($editurl->out(false));
    }
}
...
if ($item->id) {
    $PAGE->navbar->add(get_string('edit_item', 'facultyfeedback'));
} else {
    $PAGE->navbar->add(get_string('add_item', 'facultyfeedback'));
}
...
if ($typ!='label') {
    $PAGE->requires->js('/mod/facultyfeedback/facultyfeedback.js');
