...
$url = new moodle_url('/mod/facultyfeedback/show_entries.php', array('id'=>$cm->id, 'do_show'=>$do_show));
...
if(time()<$facultyfeedback->timeclose)
{
	print_error(' : You can view the report after '.date(" M jS, Y", $facultyfeedback->timeclose));
}
...
if ($do_show == 'showoneentry') {
    ...
    $facultyfeedbackcompleted = $DB->get_record('facultyfeedback_completed', $params); //arb
}
...
if(has_capability('mod/facultyfeedback:viewreports', $context) && time()>$facultyfeedback->timeclose) {
        ...
        $baseurl = new moodle_url('/mod/facultyfeedback/show_entries.php');
        ...
        if (has_capability('mod/facultyfeedback:deletesubmissions', $context)) {
            $tablecolumns[] = 'deleteentry';
            $tableheaders[] = '';
        }

        $table = new flexible_table('facultyfeedback-showentry-list-'.$course->id);
        ...
        $matchcount = facultyfeedback_count_complete_users($cm, $usedgroupid);
        ...
        $students = facultyfeedback_get_complete_users($cm, $usedgroupid, $where, $params, $sort, $startpage, $pagecount);
        $str_analyse = get_string('analysis', 'facultyfeedback');
        $str_complete = get_string('completed_facultyfeedbacks', 'facultyfeedback');
        $str_course = get_string('course');

        $completed_fb_count = facultyfeedback_get_completeds_group_count($facultyfeedback, $mygroupid);
        if ($facultyfeedback->course == SITEID) {
            $analysisurl = new moodle_url('/mod/facultyfeedback/analysis_course.php', array('id'=>$id, 'courseid'=>$courseid));
            echo $OUTPUT->box_start('mdl-align');
            echo '<a href="'.$analysisurl->out().'">';
            echo $str_course.' '.$str_analyse.' ('.$str_complete.': '.intval($completed_fb_count).')';
            echo '</a>';
            echo $OUTPUT->help_icon('viewcompleted', 'facultyfeedback');
            echo $OUTPUT->box_end();
        } else {
            $analysisurl = new moodle_url('/mod/facultyfeedback/analysis.php', array('id'=>$id, 'courseid'=>$courseid));
            echo $OUTPUT->box_start('mdl-align');
            echo '<a href="'.$analysisurl->out().'">';
    ...
    if (has_capability('mod/facultyfeedback:viewreports', $context)) {
        ...
        if (!$students) {
            ...
            echo print_string('non_anonymous_entries', 'facultyfeedback');
            ...
            foreach ($students as $student) {
                ...
                $completed_count = $DB->count_records('facultyfeedback_completed', $params);
                if ($completed_count > 0) {
                    ...
                    $facultyfeedbackcompleted = $DB->get_record('facultyfeedback_completed', $params);
                    $showentryurl_params = array('userid'=>$student->id, 'do_show'=>'showoneentry');
                    $showentryurl = new moodle_url($url, $showentryurl_params);
                    $showentrylink = '<a href="'.$showentryurl->out().'">'.userdate($facultyfeedbackcompleted->timemodified).'</a>';
                    $data[] = $showentrylink;

                    //link to delete the entry
                    if (has_capability('mod/facultyfeedback:deletesubmissions', $context)) {
                        $delete_url_params = array('id' => $cm->id,
                                            'completedid' => $facultyfeedbackcompleted->id,
                                            'do_show' => 'showoneentry');

                        $deleteentryurl = new moodle_url($CFG->wwwroot.'/mod/facultyfeedback/delete_completed.php', $delete_url_params);
                        $deleteentrylink = '<a href="'.$deleteentryurl->out().'">'.
                                                get_string('delete_entry', 'facultyfeedback').
                                            '</a>';
        ...
        <table width="100%">
            <tr>
                <td align="left" colspan="2">
                    ...
                    $facultyfeedback_completeds_count = $DB->count_records('facultyfeedback_completed', $params);
                    print_string('anonymous_entries', 'facultyfeedback');
                    echo '&nbsp;('.$facultyfeedback_completeds_count.')';
                    ?>
                </td>
                <td align="right">
                    <?php
                        ...
                        echo $OUTPUT->single_button($aurl, get_string('show_entries', 'facultyfeedback'));
                    ?>
                </td>
            </tr>
        </table>
...
if ($do_show == 'showoneentry') {
    ...
    if (is_array($facultyfeedbackitems)) {
        ...
        if ($facultyfeedbackcompleted) {
            echo $OUTPUT->heading(userdate($facultyfeedbackcompleted->timemodified).' ('.fullname($usr).')', 3);
        } else {
            echo $OUTPUT->heading(get_string('not_completed_yet', 'facultyfeedback'), 3);
        }
        ...
        foreach ($facultyfeedbackitems as $facultyfeedbackitem) {
            //get the values
            $params = array('completed'=>$facultyfeedbackcompleted->id, 'item'=>$facultyfeedbackitem->id);
            ...
            if ($facultyfeedbackitem->typ != 'pagebreak') {
                ...
                if (isset($value->value)) {
                    facultyfeedback_print_item_show_value($facultyfeedbackitem, $value->value);
                } else {
                    facultyfeedback_print_item_show_value($facultyfeedbackitem, false);
                }
                echo $OUTPUT->box_end();
            }
            echo $OUTPUT->box_end();
