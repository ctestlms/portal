...
class enrol_self_plugin extends enrol_plugin {
    ...
    public function enrol_page_hook(stdClass $instance) {
	 ...
	 //Checking Course registration limit//Added By Hina Yousuf
                ...
                $class=$DB->get_record_sql("SELECT name,id from {course_categories} ct WHERE id =(SELECT parent from {course_categories} cat WHERE id =(SELECT category from {course} c where id=$course->id))");
                if(strstr("EMBA",$USER->user_subgroup)==true && strstr("EMBA",$class->name)==true){
		 ...
		 foreach($courses as $course){
			 ...
			 return $OUTPUT->notification("You have exceeded the course registration limit of batch ".$string." .In order to add a new course you have to drop some other course");
                }
        }
