...
class facultyfeedback_item_captcha extends facultyfeedback_item_base {
    protected $type = "captcha";
    private $commonparams;
    private $item_form = false;
    private $item = false;
    private $facultyfeedback = false;
    ...
    public function build_editform($item, $facultyfeedback, $cm) {
        ...
        $editurl = new moodle_url('/mod/facultyfeedback/edit.php', array('id'=>$cm->id));

        //ther are no settings for recaptcha
        if (isset($item->id) AND $item->id > 0) {
            notice(get_string('there_are_no_settings_for_recaptcha', 'facultyfeedback'), $editurl->out());
            exit;
        }

        //only one recaptcha can be in a facultyfeedback
        $params = array('facultyfeedback' => $facultyfeedback->id, 'typ' => $this->type);
        if ($DB->record_exists('facultyfeedback_item', $params)) {
            notice(get_string('only_one_captcha_allowed', 'facultyfeedback'), $editurl->out());
            exit;
        }
        ...
        $this->item->facultyfeedback = $facultyfeedback->id;
        $this->item->template = 0;
        $this->item->name = get_string('captcha', 'facultyfeedback');
        $this->item->label = get_string('captcha', 'facultyfeedback');
    ...
    public function save_item() {
        ...
        if (empty($this->item->id)) {
            $this->item->id = $DB->insert_record('facultyfeedback_item', $this->item);
        } else {
            $DB->update_record('facultyfeedback_item', $this->item);
        }

        return $DB->get_record('facultyfeedback_item', array('id'=>$this->item->id));
    }
    ...
    public function print_item_preview($item) {
        ...
        if ($facultyfeedbackid > 0) {
            $facultyfeedback = $DB->get_record('facultyfeedback', array('id'=>$facultyfeedbackid));
            $cm = get_coursemodule_from_instance("facultyfeedback", $facultyfeedback->id, $facultyfeedback->course);
            if ($cm) {
                $cmid = $cm->id;
            }
        }

        $requiredmark = '<span class="facultyfeedback_required_mark">*</span>';
    ...
    public function print_item_complete($item, $value = '', $highlightrequire = false) {
        ...
        if (isset($SESSION->facultyfeedback->captchacheck) AND
                $SESSION->facultyfeedback->captchacheck == $USER->sesskey AND
                $value == $USER->sesskey) {
    ...
    public function print_item_show_value($item, $value = '') {
        ...
        if ($facultyfeedbackid > 0) {
            $facultyfeedback = $DB->get_record('facultyfeedback', array('id'=>$facultyfeedbackid));
            if ($cm = get_coursemodule_from_instance("facultyfeedback", $facultyfeedback->id, $facultyfeedback->course)) {
                $cmid = $cm->id;
            }
        }
    ...
    public function check_value($value, $item) {
        ...
        if ($response->is_valid) {
            $SESSION->facultyfeedback->captchacheck = $USER->sesskey;
            return true;
        }
        unset($SESSION->facultyfeedback->captchacheck);
