...
//print_r($this->columns);
    ...
    public function print_continue() {
        ...
        if (!$this->userkey) {      // this button should trigger a download prompt
             echo $OUTPUT->single_button(new moodle_url('/grade/export/' . $this->plugin . '/export.php', $params), get_string('downloaddetailed', 'grade/export/pdf'));
             echo $OUTPUT->single_button(new moodle_url('/grade/export/pdf/export.php', $params), get_string('downloadsummarized', 'grade/export/pdf'));                   
         } else {
            $paramstr = '';
            $sep = '?';
