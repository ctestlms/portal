...
$url = new moodle_url('/mod/facultyfeedback/show_entries_anonym.php', array('id'=>$id));
// if ($userid !== '') {
...
if (!$showcompleted) {
    ...
    if ($facultyfeedbackcompleteds = $DB->get_records('facultyfeedback_completed', $params, 'random_response')) {
        //then get all of the anonymous records and go through them
        $params = array('facultyfeedback'=>$facultyfeedback->id, 'anonymous_response'=>FEEDBACK_ANONYMOUS_YES);
        $facultyfeedbackcompleteds = $DB->get_records('facultyfeedback_completed', $params, 'id'); //arb
        shuffle($facultyfeedbackcompleteds);
        $num = 1;
        foreach ($facultyfeedbackcompleteds as $compl) {
            $compl->random_response = $num;
            $DB->update_record('facultyfeedback_completed', $compl);
            $num++;
        }
    }

    $params = array('facultyfeedback'=>$facultyfeedback->id, 'anonymous_response'=>FEEDBACK_ANONYMOUS_YES);
    $facultyfeedbackcompletedscount = $DB->count_records('facultyfeedback_completed', $params);

    // preparing the table for output
    $baseurl = new moodle_url('/mod/facultyfeedback/show_entries_anonym.php');
    ...
    $table = new flexible_table('facultyfeedback-showentryanonym-list-'.$course->id);
    ...
    $matchcount = $facultyfeedbackcompletedscount;
    ...
    $facultyfeedbackcompleteds = $DB->get_records('facultyfeedback_completed',
                                        array('facultyfeedback'=>$facultyfeedback->id, 'anonymous_response'=>FEEDBACK_ANONYMOUS_YES),
                                        'random_response',
                                        'id,random_response',
                                        $startpage,
                                        $pagecount);

    if (is_array($facultyfeedbackcompleteds)) {
        echo $OUTPUT->box_start('generalbox boxaligncenter boxwidthwide');
        echo $OUTPUT->heading(get_string('anonymous_entries', 'facultyfeedback'), 3);
        foreach ($facultyfeedbackcompleteds as $compl) {
            $data = array();

            $data[] = get_string('response_nr', 'facultyfeedback').': '. $compl->random_response;

            //link to the entry
            $showentryurl = new moodle_url($baseurl, array('showcompleted'=>$compl->id));
            $showentrylink = '<a href="'.$showentryurl->out().'">'.get_string('show_entry', 'facultyfeedback').'</a>';
            $data[] = $showentrylink;
            ...
            if (has_capability('mod/facultyfeedback:deletesubmissions', $context)) {
                ...
                $deleteentryurl = new moodle_url($CFG->wwwroot.'/mod/facultyfeedback/delete_completed.php', $delet_url_params);
                $deleteentrylink = '<a href="'.$deleteentryurl->out().'">'.get_string('delete_entry', 'facultyfeedback').'</a>';
...
//print the items
if ($showcompleted) {
    $continueurl = new moodle_url('/mod/facultyfeedback/show_entries_anonym.php',
                                array('id'=>$id, 'do_show'=>''));
    ...
    $params = array('facultyfeedback'=>$facultyfeedback->id);
    $facultyfeedbackitems = $DB->get_records('facultyfeedback_item', $params, 'position');
    $facultyfeedbackcompleted = $DB->get_record('facultyfeedback_completed', array('id'=>$showcompleted));
    if (is_array($facultyfeedbackitems)) {
        $align = right_to_left() ? 'right' : 'left';

        if ($facultyfeedbackcompleted) {
            echo $OUTPUT->box_start('facultyfeedback_info');
            echo get_string('chosen_facultyfeedback_response', 'facultyfeedback');
            echo $OUTPUT->box_end();
            echo $OUTPUT->box_start('facultyfeedback_info');
            echo get_string('response_nr', 'facultyfeedback').': ';
            echo $facultyfeedbackcompleted->random_response.' ('.get_string('anonymous', 'facultyfeedback').')';
            echo $OUTPUT->box_end();
        } else {
            echo $OUTPUT->box_start('facultyfeedback_info');
            echo get_string('not_completed_yet', 'facultyfeedback');
            echo $OUTPUT->box_end();
        }
        ...
        foreach ($facultyfeedbackitems as $facultyfeedbackitem) {
            ...
            if ($facultyfeedbackitem->typ != 'pagebreak') {
                ...
                facultyfeedback_print_item_show_value($facultyfeedbackitem, $itemvalue);
                echo $OUTPUT->box_end();
            }
            echo $OUTPUT->box_end();
        }
