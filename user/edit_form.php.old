...
class user_edit_form extends moodleform {
	...
	$regex="/^[0]{1}[0-9]{10}$/i";
        //validate phone //Added By Hina Yousuf
        if (!preg_match($regex, $usernew->phone2)) {
                $errors['phone2'] =  "Incorrect Format";
        }
        //end
	 ...
	 //Added By Hina Yousuf
            ...
            if(!empty($usernew->idnumber) and $user->idnumber != $usernew->idnumber)
                {
                ...
                if ($DB->record_exists('user', array('idnumber'=>$usernew->idnumber))) {
                $errors['idnumber'] = "Registration Number already exists";
                }
                }
                //end
