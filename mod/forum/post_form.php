...
class mod_forum_post_form extends moodleform {
    ...
    function definition() {
		...
		// Added by Qurrat-ul-ain for Android SMS App (18th July, 2013)
          // if (empty($post->id) && has_capability('moodle/course:manageactivities', $coursecontext)) { // hack alert
        	// $mform->addElement('checkbox', 'smsnow', get_string('smsnow', 'forum'));
        // }
		...
		if($catergory) {
			...
			if ($school->name == "School of Electrical Engineering and Computer Science (SEECS)" ) {
				if (empty($post->id) && has_capability('moodle/course:manageactivities', $coursecontext)) { // hack alert
					$smsarray = array();
					$smsarray[] =& $mform->addElement('checkbox', 'smsnow', get_string('smsnow', 'forum'));
					$smsarray[] =& $mform->addElement('static', 'staticsms', '', '<font color="red"><i>Note:  The Subject line will be forwarded as text. </i></font>');
				}
			}
		}
