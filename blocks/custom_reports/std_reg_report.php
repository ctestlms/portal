...
$navlinks[] = array('name' => 'Course Registration Report', 'link' => null, 'type' => 'activityinstance');
$navigation = build_navigation($navlinks);
//$report=get_string('std_reg_report', 'block_custom_reports');
$report='Course Registration Report';
if(!$export)
print_header('Course Registration Report', 'Course Registration Report', $navigation, '', '', true, '', user_login_string($SITE).$langmenu);

if($courses = get_courses($categoryid, '', 'c.id, c.fullname, c.startdate, c.idnumber, c.shortname') or $export){
	$mform = new mod_custom_reports_view_attendance_report_form('std_reg_report.php', array('courses'=>$courses, 'categoryid'=>$categoryid,'report'=>$report));
	if($fromform = $mform->get_data() or $export){
		...
		foreach($courses as $course){
			...
			if((!$export AND $fromform->{'c'.$course->id}=='true') OR ($export)){
				...
				$cselected["name"][] = $course->fullname;
			        $teacher=$DB->get_record_sql($query3);
                                $cselected["teachername"][] = $teacher->id;
		...
		for($i=0; $i<count($cselected["id"]); $i++){
			...
			foreach($cselected["students"][$i] as $student){
				...
				// end
				if(!in_array($student->id, $students["userid"])){// && $student->profile_field_registrationstatus!="not joined" && $student->deleted!=1){
		...
		for(; $row_ite<count($students["userid"]); $row_ite++){
			if($students["school"][$row_ite] == 'School of Electrical Engineering and Computer Science (SEECS)') {
				if($students["reg"][$row_ite] != 'not joined') {
					...
					$table->data[$row_ite][] = $students["userid"][$row_ite];
					...
					$table->data[$row_ite][] =  "<a href='editadvanced.php?id=" . $students["userid"][$row_ite] . "&schoolid=" . $categoryid . "&batch=" . $batch . "'>Edit Profile</a>";				
				}
			}
			else {
				$table->data[$row_ite][] = $row_ite+1;
		...
		if($export){
			...
			echo '<div style="text-align: center; font-weight: bold;">Course Registration Report </div>';
...
function ExportToExcel($data) {
	...
	$filename = "course_reg_report:.xls";
	...
	$myxls =& $workbook->add_worksheet('course_reg_report');
	/// format types
