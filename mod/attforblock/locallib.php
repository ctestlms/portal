...
// params added for sorting by reg no and sub group - added by Qurrat-ul-ain
define('ATT_SORT_ID', 1);
define('ATT_SORT_LASTNAME', 2);
define('ATT_SORT_FIRSTNAME', 3);
define('ATT_SORT_SUBGROUP', 4);
define('ATT_SORT_FN', 5);
...
class attforblock_permissions {
	...
	public function can_unmark() {
        if (is_null($this->canunmark)) {
            $this->canunmark = has_capability('mod/attforblock:unmarkattendances', $this->context);
        }

        return $this->canunmark;
    }
...
class att_page_with_filter_controls {
    ...
    protected $defaultview      = ATT_VIEW_ALL;
...
class att_take_page_params {
	...
	if (!isset($this->sort)) $this->sort = ATT_SORT_ID; // added by Qurrat-ul-ain
    ...
    public function get_significant_params() {
        ...
        if ($this->sort != ATT_SORT_ID) $params['sort'] = $this->sort; // added by Qurrat-ul-ain
...
class attforblock {
    ...
    public function take_from_form_data($formdata) {
		...
		//  Added by Aftab Aslam  (14th may, 2014)  // 
		$sessionTime = $DB->get_record_sql("select sessdate,duration from  {attendance_sessions}  where id=".$this->pageparams->sessionid);
		$sessionStatus = $DB->get_record_sql("select attendenceStatus from  {attendance_sessions}  where id=".$this->pageparams->sessionid." and lasttaken is not null");
		 
	 
		$sessionStartime = $sessionTime->sessdate;
		$sessionEndtime = $sessionTime->sessdate+$sessionTime->duration;
		
		$attendenceStatus =0;
		
		if(   ($sessionStartime <= $now) && ($now <= $sessionEndtime)){
	 			$attendenceStatus =1;
			}
		 
		 
		if( ($sessionStatus->attendencestatus) == 1){
			 
				$attendenceStatus =1;	
		} 
		 
		
		$topic = $formdata->topicname;   // Added by Qurrat-ul-ain Babar (19th Dec, 2013)
		...
		$rec->attendenceStatus = $attendenceStatus;
		//print_r($rec);exit;
	 
		if($topic != "")
		{
			$rec->topicname = $topic;   // Added by Qurrat-ul-ain Babar (19th Dec, 2013)
		}
        $DB->update_record('attendance_sessions', $rec);
		
		$DB->execute("UPDATE  {attendance_sessions}  SET attendenceStatus =".$attendenceStatus." WHERE  id=".$this->pageparams->sessionid);
    ...
    public function get_users($groupid = 0) {
        ...
        $userfields = user_picture::fields('u').',u.username'.',u.idnumber'.',u.user_subgroup';
		...
		// added by Qurrat-ul-ain for sorting by reg no and sub group
        if (isset($this->pageparams->sort) and ($this->pageparams->sort == ATT_SORT_LASTNAME)) {
            $orderby = "u.lastname ASC, u.firstname ASC";
        }
		else if (isset($this->pageparams->sort) and ($this->pageparams->sort == ATT_SORT_FIRSTNAME)) {
            $orderby = "u.firstname ASC, u.lastname ASC";
        }
		else if (isset($this->pageparams->sort) and ($this->pageparams->sort == ATT_SORT_SUBGROUP)) {
            $orderby = "u.user_subgroup ASC";
        }
        else {
            $orderby = "u.idnumber ASC";
        }
        ...
        if (!empty($users)) {
            ...
            //CONTRIB-3549
            $sql = "SELECT ue.userid, ue.status, ue.timestart, ue.timeend, ue.timecreated
            ...
            foreach ($users as $user) {
				...
				$users[$user->id]->registrationstatus = $record->profile_field_registrationstatus;
                ...
                $users[$user->id]->enrolmentcreated = $enrolmentsparams[$user->id]->timecreated;
            }
        }
    ...
    public function get_user($userid) {
        ...
        $user->enrolmentcreated = $enrolmentsparams->timecreated;
    ...
    /* public function get_user_statuses_stat($userid) {
        ...
        if (!array_key_exists($userid, $this->userstatusesstat)) {
			...
			print_r($DB->get_records_sql($qry, $params));
	...
	// updated function for fetching lecture description and calculating credit hour based %s - updated by Qurrat-ul-ain Babar (20th Dec, 2013)
	public function get_user_statuses_stat($userid) {
        global $DB;
		...
		$sql1 = "SELECT ue.userid, ue.status, ue.timestart, ue.timeend, ue.timecreated
				 ...
				 WHERE ue.userid = :uid
					   AND e.courseid = :courseid";
			  //GROUP BY ue.userid, ue.status, ue.timestart, ue.timeend";
		$params1 = array('uid'=> $userid, 'courseid'=>$courseid);
        ...
        if (!array_key_exists($userid, $this->userstatusesstat)) {
			...
			if ($this->pageparams->startdate && $this->pageparams->enddate) {
				$where = "ats.attendanceid = :aid AND ats.sessdate >= :csdate AND 
						  ats.sessdate >= :sdate AND ats.sessdate < :edate AND
						  al.studentid = :uid";
			} else {
			...
			$this->userstatusesstat[$userid] = $statsarray;
        }
        return $this->userstatusesstat[$userid];
    }
    ...
    public function update_sessions_duration($sessionsids, $duration, $description) {
        global $DB;
...
// gets attendance status for a student, returns count ---- Added by Qurrat-ul-ain Babar

/* function att_get_status_count($attid, $userid, $coursestartdate, $statusid = 0) {
    global $DB;
    ...
    //echo $DB->count_records_sql($qry, $params);
...
function att_get_user_taken_sessions_count($attid, $coursestartdate, $userid) {
    ...
    return $DB->count_records_sql($qry, $params); */
	$statuscount = array();
	$statuscount = att_get_user_statuses_stat($attid, $coursestartdate, $userid);
	$takencount = 0;
	
	foreach($statuscount as $value)
	{
		$takencount += $value->stcnt;
	}
	return $takencount;
}
...
function att_get_user_statuses_stat($attid, $coursestartdate, $userid) {
	...
	$qry = "SELECT ats.id, al.studentid, al.statusid, ats.description, ats.sessdate
			  FROM {attendance_log} al
			  JOIN {attendance_sessions} ats
				ON al.sessionid = ats.id
			 WHERE $where";
			...
			foreach ($data as $status) {
				...
				$acrod = $DB->get_record_sql($acro);
				
				// if condition added for not counting unmarked sessions in total - Added by Qurrat-ul-ain (30th Sep, 2014)
				if ($acrod->acronym == "UM") {
					// Session unmarked so do nothing
				} 
				else {
					$classtype = $status->description;
					switch ($classtype) {
...
function get_firstmonday($month, $year) {
    ...
    $num = mktime(0, 0, 0, $month, date("d"), $year);
	$lockdate = mktime(0, 0, 0, $month, 27, $year);
	
	echo userdate($num, get_string('strftimedmy', 'attforblock'));
	echo "<br/>";
	echo userdate($lockdate, get_string('strftimedmy', 'attforblock'));
	echo "<br/>";
	
    /* switch ($school->name) {
        case ($school->name == "School of Electrical Engineering and Computer Science (SEECS)" ):
            $lockdate = 5;            
            break;
        case ($school->name == "Atta-ur-Rehman School of Applied Biosciences (ASAB)" ):
            $lockdate = 5;
            break;
        case ($school->name == "Centre for Advanced Mathematics & Physics (CAMP)"   ):
            $lockdate = 5;
            break;
        case ($school->name == "Centre for Energy Systems (CES)" ):
            $lockdate = 5;
            break;
        case ($school->name == "College of Electrical & Mechanical Engineering (E&ME)" ):
            $lockdate = 20;
            break;
        case ($school->name == "Institute of Environmental Science & Engineering (IESE)" ):
            $lockdate = 5;
            break;
        case ($school->name == "Institute of Geographical Information Systems (IGIS)" ):
            $lockdate = 5;
            break;
        case ($school->name == "Military College of Engineering (MCE)" ):
            $lockdate = 20;
            break;
         case ($school->name == "Military College of Signals (MCS)" ):
            $lockdate = 20;
            break;
         case ($school->name == "NUST Business School (NBS)" ):
            $lockdate = 5;
            break;
         case ($school->name == "National Institute of Transportation (NIT)" ):
            $lockdate = 5;
            break;
         case ($school->name == "NUST Institute of Civil Engineering (NICE)" ):
            $lockdate = 5;
            break;
         case ($school->name == "Pakistan Navy Engineering College (PNEC)" ):
            $lockdate = 20;
            break;
         case ($school->name == "Research Center for Modeling and Simulation (RCMS)" ):
            $lockdate = 5;
            break;
         case ($school->name == "School of Chemical and Materials Engineering (SCME)" ):
            $lockdate = 5;
            break;
         case ($school->name == "School of Art, Design and Architecture (SADA)" ):
            $lockdate = 5;
            break;
         case ($school->name == "School of Electrical Engineering and Computer Science (SEECS)" ):
            $lockdate = 5;
            break;
         case ($school->name == "School of Mechanical and Manufacturing Engineering (SMME)" ):
            $lockdate = 5;
            break;
         case ($school->name == "Center for Counseling and Career Advisory (C3A)" ):
            $lockdate = 5;
            break;
         case ($school->name == "School of Social Sciences and Humanities (NSSSH)" ):
            $lockdate = 20;
            break;
        case ($school->name == "College of Aeronautical Engineering (CAE)" ):
            $lockdate = 20;
            break;
        case ($school->name == "Army Medical College (AMC)" ):
            $lockdate = 20;
            break;
    ...
    if ($num > $lockdate) {
		echo "Current date is greater than lock date<br/>";
        return mktime(0, 0, 0, $month, 1, $year);
	}
    else {
		echo "current date is smaller than lock date<br/>";
