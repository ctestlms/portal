...
if (isset($_POST['report']) or $export) {
    ...
    if ($id != 0) {     //If the school is selected not NUST
        ...
        $sql = "SELECT u.id
				...
				WHERE r.id=19 and u.id= $USER->id and u.user_subgroup='$department'";
    ...
    if ($hod == 1 || $observer == 1 || $admin == 1) {
		...
		// Last edited by - added by Qurrat-ul-ain Babar (7th May, 2014)
		$table->head[] = 'Last Edited By';
        $table->align[] = 'center';
        $table->size[] = '';
            ...
            foreach ($courses as $course) {
                ...
                if ($teacher = $DB->get_records_sql($sql)) {
                    ...
                    if (!in_array($course->courseid, $unique_courses)) {
						...
						$coursetotal = $DB->get_record('grade_items', array('courseid' => $course->courseid, 'itemtype' => 'course'));
						$lastmodified = $DB->get_records('grade_grades', array('itemid' => $coursetotal->id));
						 ...
						 foreach($lastmodified as $lm) {
							...
							if($timemod == "") {
								...
								//$time = userdate($timemod, get_string('strftimedmy', 'attforblock'))."-".userdate($timemod, get_string('strftimehm', 'attforblock')); // time when modified
								$time = date(" M jS, Y - H:i", $timemod);
							}
							else {
						...
						// Last edited by - added by Qurrat-ul-ain Babar (7th May, 2014)
						if($timemod != "")
							$table->data[$i][] = $username." at ".$time;
						else
							$table->data[$i][] = "-";
                        $table->data[$i][] = ($submission->facultytimestamp != "") ? date(" M jS, Y - H:i", $submission->facultytimestamp) : "Not Submitted"; // updated by Qurrat-ul-ain to show hours and minutes (10th Jan, 2014)
                        $table->data[$i][] = ($submission->hodtimestamp != "") ? date(" M jS, Y - H:i", $submission->hodtimestamp) : "Not Submitted"; // updated by Qurrat-ul-ain to show hours and minutes (10th Jan, 2014)
                        $table->data[$i][] = ($submission->seniorhodtimestamp != "") ? date(" M jS, Y - H:i", $submission->seniorhodtimestamp) : "Not Submitted"; // updated by Qurrat-ul-ain to show hours and minutes (10th Jan, 2014)
                        $table->data[$i][] = ($submission->deantimestamp != "") ? date(" M jS, Y - H:i", $submission->deantimestamp) : "Not Submitted"; // updated by Qurrat-ul-ain to show hours and minutes (10th Jan, 2014)
                    }
                }
            }
        }
...
/// format types
	...
	$myxls->write_string(4, 0, "S. No", $formatbc);
	$myxls->write_string(4, 1, "Teacher", $formatbc);
	$myxls->write_string(4, 2, "Course", $formatbc);
	$myxls->write_string(4, 3, "Last Edited By", $formatbc);
	$myxls->write_string(4, 4, "Teacher Submission", $formatbc);
	$myxls->write_string(4, 5, "Hod Submission", $formatbc);
	$myxls->write_string(4, 6, "Senior Hod Submission", $formatbc);
	$myxls->write_string(4, 7, "Dean Submission", $formatbc);
...
function ImprovedTable($headings, $data) {
	...
	$content = $content . '<td width="5%" align="center"><b> S. No</b></td>';
	$content = $content . '<td width="25%" align="center"><b>Teacher</b></td>';
	$content = $content . '<td width="30%" align="center"><b>Course</b></td>';
	$content = $content . '<td width="8%" align="center"><b>Last Edited By</b></td>';
	$content = $content . '<td width="8%" align="center"><b>Teacher Submission</b></td>';
	$content = $content . '<td width="8%" align="center"><b>Hod Submission</b></td>';
	$content = $content . '<td width="8%" align="center"><b>Senior Hod Submission</b></td>';
	$content = $content . '<td width="8%" align="center"><b>Dean Submission</b></td>';
    $content = $content . '</tr>';
    //Data
    foreach ($data->data as $row) {
        $content = $content . '<tr>';
