...
$report = get_string('bulk_enrol_action', 'block_bulk_action');
$navlinks[] = array('name' => get_string('bulk_action', 'block_bulk_action'), 'link' => null, 'type' => 'activityinstance');
$navigation = build_navigation($navlinks);
print_header(get_string('bulk_enrol_action', 'block_bulk_action'), get_string('bulk_enrol_action', 'block_bulk_action'), $navigation, '', '', true, '', user_login_string($SITE) . $langmenu);
$count=false;
if (($courses && $categoryid == 2) or $export or $sort) {
    $mform = new mod_bulk_action_form('enrolbulkaction.php', array('courses' => $courses, 'categoryid' => $categoryid, 'report' => $report));
    if ($fromform = $mform->get_data()) {
		...
		if (($fromform->{'manual'} == 'Manual')) {
                    
                    $manual = $DB->get_record('enrol', array('courseid' => $course->id, 'enrol' => 'manual'), '*', MUST_EXIST);
                    $enrol_methods = $DB->get_records_sql("select * from {enrol} where courseid=$course->id and  enrol = 'cohort'");
                    foreach ($enrol_methods as $emethods) {
                        $users = $DB->get_records('user_enrolments', array('enrolid' => $emethods->id));
                        foreach ($users as $user) {
                            $user->enrolid = $manual->id;
                            $feedbackid = $DB->update_record("user_enrolments", $user);
                            // $DB->execute();
                        }
                    }
                }
                if (($fromform->{'self'} == 'true')) {
                    
                    $enrol_methods = $DB->get_records_sql("select * from {enrol} where courseid=$course->id and  enrol = 'cohort'");
                    foreach ($enrol_methods as $emethods) {
                        $users = $DB->get_records('user_enrolments', array('enrolid' => $emethods->id));
                        foreach ($users as $user) {
                            $user->enrolid = $instance->id;
                            $feedbackid = $DB->update_record("user_enrolments", $user);
                            // $DB->execute();
                        }
                    }
               ...
               // $feedbacks = $DB->get_records_sql("select * from {feedback} where name like '%$fromform->feedbacktype%' and course=$course->id ");
                    ...
                    if (empty($fromform->openenable)) {
                        $instance->enrolstartdate = $instance->enrolstartdate;
                    } else {
                        $instance->enrolstartdate = $fromform->timeopen;
                    }
                    if (empty($fromform->closeenable)) {
                        $instance->enrolenddate = $instance->enrolenddate;
                    } else {
                        $instance->enrolenddate = $fromform->timeclose;
                    }
                    $instance->customint3     = $fromform->{'users' . $course->id};
		    $instance->customint5     = $fromform->{'customint5'};
                    $instance->timemodified = time();
		    $instance->customint2=0;
		    $instance->status = 0;
                    $feedbackid = $DB->update_record("enrol", $instance);
        ...
        if (empty($fromform->openenable) && empty($fromform->openenable))  {
             echo '<div style="text-align: center; font-weight: bold;">You should select the  opening or closing date<br></div>';
        }
        if ($feedbackid == 1) {
    ...
    echo '<form method="post" action="enrolbulkaction.php?id=2" style="display: inline; margin: 0; padding: 0;">';
