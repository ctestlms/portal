...
function redirect_to_feedback_page($course, $USER) {
        global $CFG, $DB;
        ...
        if($roleid->roleid==5)
        {
            ...
            $timeAfterOneWeek = strtotime("+7 days",  $feedback->timeopen);
                if($feedback && time()>$timeAfterOneWeek && time()<= $feedback->timeclose){
                        $coursemodule=$DB->get_record_sql("SELECT id from {course_modules} cm WHERE course =$course->id and instance=$feedback->id");
        ...
        foreach ($modinfo->sections[$section->section] as $modnumber) {
		  ...
		  //Added By Hina yousuf
                ...
                $roleid=$DB->get_record_sql("SELECT roleid
                                        FROM {role_assignments} ra
                                        WHERE ra.userid =$USER->id AND contextid =$context->id");
                //if($roleid->roleid==3 && ( $instancename=="First Student Feedback" || $instancename=="Second Student Feedback"))
                if($roleid->roleid==3 &&  strstr($instancename,  "Student Feedback")==true)
                         {
                                //Do nothing 
                        }
                        else{
...
//end
             echo "<div style='color:grey;float:right;'><b>(seen by ".($views->numviews!=""?$views->numviews:"0").")</b>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</div><br>";
            }else{
...
function print_category_info($category, $depth=0, $showcourses = false) {
    ...
    if ($showcourses and $coursecount) {
        echo '<div class="categorylist clearfix level'.$depth.'">';
        $cat = '';
 ...
 * create feedback activity automatically and import question  by using the template.

 * course: course id 

 * feedbackname: name of the feedback to be used

 * templatefile: question to be imported into the feedback activity

 * feedback activity: feedback activity could be student feedback or faculty feedback
 ...
 * @param int $feedbackname

 * @param int $templatefile

 * @param $feedbackactivity
*/

function create_feedback_activity($course, $feedbackname, $templatefile, $feedbackactivity) {

    //Adding two feedback module automatically in course when course is created.
    ...
    $feedback->intro = $feedbackname;

    $feedback->timeopen = strtotime("+42 days", $courserecord->timemodified);

    $feedback->timeclose = strtotime("+14 days", $courserecord->timemodified);
   ...
   $feedback->autonumbering = 1;
    ...
    $feedback->page_after_submit = "<p>Thank you for your feedback!!!</p>";
    ...
    if ($feedbackactivity == "studentfeedback") {
    ...
    if ($feedbackactivity == "facultyfeedback") {

        $feedbackid = $DB->insert_record("facultyfeedback", $feedback);
		$feedback->id = $feedbackid;
    ...
    $xmlcontent = file_get_contents($CFG->dirroot . "/$templatefile", true);
    ...
    if ($feedbackactivity == "facultyfeedback") {
        ...
        if (!$xmldata = facultyfeedback_load_xml_datap($xmlcontent)) {

            print_error('cannotloadxml', 'facultyfeedback', 'edit.php?id=' . $id);
              }

        $importerror = facultyfeedback_import_loaded_datap($xmldata, $feedback->id);

        if (!$module = $DB->get_record("modules", array("name" => "facultyfeedback"))) {

            echo $OUTPUT->notification("Could not find facultyfeedback module!!");
...
function create_course($data, $editoroptions = NULL) {
    ...
    events_trigger('course_created', $course);
	  ...
	  if ($attendances = $DB->get_records_select("attforblock", "course = ? ", array($course->id), "id ASC")) {
...
//create feedback activity
    create_feedback_activity($course, "Student Course Evaluation", "studentcoursevaluation.xml", "studentfeedback");
    create_feedback_activity($course, "Faculty Course Overview Report", "facultycoursevaluation.xml", "facultyfeedback");

	//Added By Hina Yousuf(To create the grade category and grade items by default in course)
    create_gradebook_categories_bydefault($course);
    //end
	
	// Set course total grade category to "Sum of Grades" - added by Qurrat-ul-ain Babar (2nd June, 2014)
	global $DB;
	$coursetotal = $DB->get_record_sql("SELECT * from {grade_categories} WHERE courseid=$course->id AND parent IS NULL");
	
	$ct = new stdClass();
    $ct->id = $coursetotal->id;
    $ct->aggregation = 13;
    $DB->update_record('grade_categories', $ct);
	// end
...
/**
 * Creates the grade category and grade items by default in course
 * @param object $course course object.
 */
function create_gradebook_categories_bydefault($course) {
    global $CFG, $DB;
    $credithours=explode("+",$course->credithours);
    $theorycredithours=$credithours[0];
    $practicalcredithours=$credithours[1];
    $theorypercentage=(($theorycredithours/($theorycredithours+$practicalcredithours))*100);
    $practicalpercentage=(($practicalcredithours/($theorycredithours+$practicalcredithours))*100);

//create grade cateories and gradeitems on course creation
    if ($practicalcredithours==0) {
        $theory_catg_id = create_grade_categories("Sessionals", null, $course, 50);
    } else {
        $theory_catg_id = create_grade_categories("Theory", null, $course, $theorypercentage);
    }
    $quiz_catg_id = create_grade_categories("Quizzes", $theory_catg_id, $course, 10);
    for ($i = 1; $i <= 3; $i++) {
        create_grade_items("Quiz" . $i, $quiz_catg_id, $course, 10);
    }
    $assignment_catg_id = create_grade_categories("Assignments", $theory_catg_id, $course, 10);
    for ($i = 1; $i <= 3; $i++) {
        create_grade_items("Assignment" . $i, $assignment_catg_id, $course, 10);
    }
    $oht_catg_id = create_grade_categories("OHT / Mid", $theory_catg_id, $course, 30);
    for ($i = 1; $i <= 2; $i++) {
        create_grade_items("OHT" . $i, $oht_catg_id, $course, 50);
    }
	if ($practicalcredithours==0) { 
	}
	else {
		$final_theory_catg_id = create_grade_categories("Final(Theory)", $theory_catg_id, $course, 50);
		for ($i = 1; $i <= 1; $i++) {
			create_grade_items("Final(Theory)", $final_theory_catg_id, $course, 50);
		}
	}
    
    if ($practicalcredithours==0) {
        // $practical_catg_id = create_grade_categories("Finals", null, $course, $finalpercentage);
        $labs_catg_id = create_grade_categories("ESE", null, $course, 50);
        create_grade_items("ESE", $labs_catg_id, $course, 60);
    } else {
        $practical_catg_id = create_grade_categories("Practical", null, $course, $practicalpercentage);
        $labs_catg_id = create_grade_categories("Lab Task (Practical)", $practical_catg_id, $course, 13);
        for ($i = 1; $i <= 3; $i++) {
            create_grade_items("Lab" . $i, $labs_catg_id, $course, 13);
        }
        $final_practical_catg_id = create_grade_categories("Final (Practical)", $practical_catg_id, $course, 10);
        for ($i = 1; $i <= 1; $i++) {
            create_grade_items("Final (Practical)", $final_practical_catg_id, $course, 10);
        }
    }
}
...
/**
 * Creates the grade category by default in course

 * @param varchar $category_name a category name .
 * @param integer $parentcategory a category id for parent category.
 * @param object $course course object.
 * @param integer $maxgrade maxgrade(percentage) of the category.
 * @return int category id
 */
function create_grade_categories($category_name, $parentcategory, $course, $maxgrade = null) {
    global $CFG, $DB;
    require_once $CFG->dirroot . '/grade/lib.php';
    require_once $CFG->dirroot . '/grade/report/lib.php';
    $grade_category = new grade_category(array('courseid' => $course->id), false);
    $grade_category->apply_default_settings();
	$grade_category->grademax = 11;
...
/**
 * Creates the grade items under grade category by default in course

 * @param varchar $item_name a category name .
 * @param integer $parentcategory a category id for parent category.
 * @param object $course course object.
 * @param integer $maxgrade maxgrade(percentage) of the grade item.
 */
function create_grade_items($item_name, $category, $course, $maxgrade = null) {
    global $CFG, $DB;
    require_once $CFG->dirroot . '/grade/lib.php';
    require_once $CFG->dirroot . '/grade/report/lib.php';
    $grade_item = new grade_item(array('courseid' => $course->id, 'itemtype' => 'manual'), false);
    ...
    $data->gradetype = 1;
    if ($maxgrade != null)
        $data->grademax = $maxgrade;
    else {
        $data->grademax = 10;
    }
    $data->grademin = 0; // updated by Qurrat-ul-ain to set default minimum garde to zero on course creation
    // $data->grademin = 1;
    $data->display = 1;
    $data->timecreated = time();
...
// Handle null decimals value
    ...
    if (empty($grade_item->id)) {
        $grade_item->categoryid = $category;
