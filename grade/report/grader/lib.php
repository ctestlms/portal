...
//// SQL-RELATED
    ...
    public function load_users() {
       ...
       // $userfields = user_picture::fields('u', get_extra_user_fields($this->context));
        $userfields = "u.id,u.picture,u.firstname,u.lastname,u.imagealt,u.idnumber";
    ...
    */
    ...
    public function get_left_rows() {
		...
		// Show serial number in report - added by Qurrat-ul-ain (7th Jan, 2014)
		$snoheader = new html_table_cell();
		//$snoheader->attributes['class'] = 'header';
        // $snoheader->scope = 'col';
        // $snoheader->header = true;
		$snoheader->text = "#";
		
		$headerrow->cells[] = $snoheader;
        ...
        foreach ($extrafields as $field) {
             //Added By Hina Yousuf(Hide email and username field)
   	    if ($field == "username") {
                continue;
            }
            if ($field == "email") {
        ...
        foreach ($this->users as $userid => $user) {
			...
			// Show serial number in report - added by Qurrat-ul-ain (7th Jan, 2014)
			$snocell = new html_table_cell();
			$snocell->text = $i;
			$userrow->cells[] = $snocell;
            ...
            foreach ($this->gtree->items as $itemid=>$unused) {
                ...
                if (in_array($itemid, $unknown)) {
					...
					if ($item->is_course_item())
						$gradeval = ROUND($grade->finalgrade);
                    else
                ...
                if ($item->needsupdate) {
                    ...
                    //        $itemcell->text .= html_writer::tag('span', format_float($gradeval, $decimalpoints), array('class'=>"gradevalue$hidden$gradepass"));
							if($grade->is_locked())
                                $itemcell->text .=html_writer::tag('span', $gradeval, array('class'=>""));
                            else
								$itemcell->text .=html_writer::tag('span', $gradeval, array('class'=>"gradevalue$hidden$gradepass"));
						}
                    }
...
//            $html .= html_writer::table($fulltable);
    ...
    public function get_right_avg_row($rows=array(), $grouponly=false) {
        ...
        if ($showaverages) {
            ...
            $sql = "SELECT g.itemid, SUM(ROUND(g.finalgrade)) AS sum, g.excluded
                      FROM {grade_items} gi
            ...
            $sql = "SELECT gi.id, COUNT(DISTINCT u.id) AS count, g.excluded
                      FROM {grade_items} gi
                      CROSS JOIN {user} u
                      JOIN ($enrolledsql) je
                           ON je.id = u.id
                     ...
                     WHERE gi.courseid = :courseid
                           ...
                           AND ra.contextid ".get_related_contexts_string($this->context)."
                           AND u.deleted = 0
                           AND (g.id IS NULL OR g.excluded !=0)
                           $groupwheresql
                  GROUP BY gi.id";
