...
$year = date('Y');
    if (!$export) {
		/// AFTAB ASLAM - 2-9-2014
		$cohorts = "SELECT   distinct(name),id  FROM `mdl_cohort`  ";
		$cohortslist = $DB->get_records_sql($cohorts);
        /////////////

    echo '<div style="text-align: center; font-weight: bold; font-size: 20px; border:1px solid #CCC;">Students Registration Report <br></div>';
 echo '<br/><br/>';
        //	echo '<div style="text-align: center; font-weight: bold;">Department:&nbsp;'.$department .'<br></div>';
        echo '<div style="text-align: center; padding-left: 20px; margin: 10px 0; border:1px solid #CCC; height:200px;">';
						//<div style="float:left; width:480px;">
						echo '<form method="post" style="display: inline; margin: 0; padding: 0;">';
						
/*				echo '<b style="vertical-align:top">Select :</b>
<select multiple="multiple" name="cohorts[]" style="height:100px">';
    		foreach($cohortslist as $cohorts) {
			echo '<option value="'.$cohorts->name.'">'.$cohorts->name.'</option>';
			}	
echo '</select>

<span style="vertical-align:top;">	<input name="submit" type="submit" value="Submit" /></span>
</div>';*/

    // echo  '<div style=" float:left;width:380px; margin-left:10px;">';		
        echo "<b>Download:</b>";
       ...
       // echo '<option value="Excel">Download in Excel Format </option>';
        ...
        echo '<input type="hidden" name="catg" value="year."'.$year.' />';
	...
	$cond='';
 //// submiting the submit button//
 if (!$export){
	  if (isset($_POST['submit'])) {
 		$cond = '';
		 foreach($_POST['cohorts'] as $cohorts) {
			 $cond .="'".$cohorts."',";
			 }
...
/*	if($cond!='') {
	...
	FROM `mdl_cohort`  WHERE name IN( ".$cond." ) ";
	}else {
	 $group = "SELECT   LEFT(`name`, LOCATE('-', `name`)-1) AS name FROM `mdl_cohort`  order by id desc limit 2  ";
	}*/
 
	
	    $group = "SELECT    LEFT(`name`, LOCATE('-', `name`)-1) AS name  FROM `mdl_cohort`  
   WHERE( name LIKE '%BESE-5%'  OR name LIKE '%BSCS-4%'    OR name LIKE '%BEE-6%'   OR name LIKE '%MSITE-2%'  
  OR name LIKE '%MSIT-15%'     OR name LIKE '%MSIS-7%'  OR name LIKE '%MSEE-6%'
  OR name LIKE '%MSCS-4%'  
...
/*	$groupms = new stdClass();
    ...
    if (!$class->name == '') {
		  ...
		  $sql = "SELECT COUNT(rf.admissiontype) AS type
				...
				JOIN {user_info_data} uid ON  uid.userid = rf.userid 
				WHERE c.name LIKE '%$class->name%'
				AND uid.data IN( 'joined' , 'pending') ";
	   ...
	   print_r($rec2);*/
	   	...
	   	$sql3 = "SELECT COUNT(cm.id) AS users
              FROM {cohort_members} cm 
              JOIN {cohort} c ON c.id = cm.cohortid 
			  JOIN {regform} rf on rf.userid = cm.userid
              WHERE c.name LIKE  '%$class->name%'
				  ";
		  	
	     $registeredtotal = $DB->get_record_sql($sql3);
	  
	  // echo $sql3;
	 //  print_r($registeredtotal);
    
   
        $totalcount = $rec2->users;
...
/*	      $join = $rec->joi ;
		...
		$tot = $total - $pend;*/
		$notregistered = $totalcount - $registeredtotal->users;
      
		$table1->data[$i][] = $class->name;
		$table1->data[$i][] = $totalcount;
		$table1->data[$i][] =  $registeredtotal->users;
        ...
        $table1->data[$i][] = $male_count->type;
        $table1->data[$i][] = $female_count->type;
        $table1->data[$i][] = $registeredtotal->users-$totalregistered;
	...
	// AFTAB ASLAM - 1-9-2014
    ///////////////////////////////////////////////Getting the Data for the New table  TABLE 1 //////////////////////////////////////////////
...
/*    if($cond!='') {
	$sql2 = "SELECT name,id FROM {cohort}  WHERE name IN( ".$cond." )";
	}else {*/
		$sql2 = "SELECT name,id FROM {cohort}  WHERE( name LIKE '%BESE-5%'  OR name LIKE '%BSCS-4%'    OR name LIKE '%BEE-6%'   OR name LIKE '%MSITE-2%'  
  					OR name LIKE '%MSIT-15%'     OR name LIKE '%MSIS-7%'  OR name LIKE '%MSEE-6%'
 				 OR name LIKE '%MSCS-4%' ) 
    ";
	/*}*/
   ...
   foreach ($classlist as $class) {
    ...
    if(!$class->name == ''){
		...
		JOIN {cohort_members} cm ON rf.userid = cm.userid
		JOIN {user_info_data} uid ON  uid.userid = rf.userid 
		WHERE cm.cohortid = $class->id AND uid.data IN( 'joined' , 'pending') ";
       ...
       // $table->data[$i][] = $i;
		...
		$tempClass = explode("-", $tempClass);
		$class=$tempClass[0]; // piece1
		$section =  $tempClass[1]; // piece2
		
		$table->data[$i][] = $class;
		$table->data[$i][] =  $section;
		$table->data[$i][] = $totalregistered;
		$table->data[$i][] =  $male_count->type;
		$table->data[$i][] =   $female_count->type;
...
/// format types
	...
	$year = date('Y');
    $myxls->write(0, 2, "Students Strength Report (Year ".$year." Fall)", $formatbc);
